...
test("@desktop сохранение менеджером подтверждает состав, доставку и открывает оплату", async ({ page }) => {
  ...
  // В редакторе больше нет отдельной галочки подтверждения: сохранить = подтвердить.
