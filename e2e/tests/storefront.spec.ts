...
test("@desktop корзина ведёт в отдельный пошаговый checkout", async ({ page }) => {
  ...
  await expect(checkout.locator('img[src*="checkout-summary-art"]')).toHaveCount(0);
  await expect(checkout.getByText("Способ доставки")).toBeHidden();
  await checkout.getByRole("button", { name: /Продолжить/ }).click();
  await expect(checkout.locator("#name-error")).toBeVisible();
  await expect(checkout.getByLabel("Имя")).toBeFocused();
  ...
  await checkout.getByRole("button", { name: /Назад/ }).click();
  await expect(checkout.getByLabel("Имя")).toHaveValue("Мария");
  await expect(checkout.getByLabel("Телефон")).toHaveValue("+7 900 123-45-67");
  await checkout.getByRole("button", { name: /Продолжить/ }).click();
  await checkout.getByRole("button", { name: /Продолжить/ }).click();
