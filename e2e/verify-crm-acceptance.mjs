...
try {
 for (const role of ['owner','manager']) {
  ...
  for(const width of [1440,390]) {
   ...
   await page.goto('/admin?section=returns');await expect(page.getByRole('heading',{name:'Возвраты растений',exact:true})).toBeVisible();await expect(page.getByText('Нужна финансовая сверка',{exact:true})).toBeVisible();await page.getByRole('button',{name:'Готово к продаже',exact:false}).click();await expect(page.getByText('Продажа связана',{exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Создать черновик поступления',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Принять возврат',exact:false})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(1);await page.screenshot({path:`${out}/${role}-stage09-returns-${width}.png`,fullPage:true});
