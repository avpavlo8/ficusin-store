...
try {
 for (const role of ['owner','manager']) {
  ...
  if(role==='manager') {
   for(const path of ['/admin?section=procurement','/admin?section=marketplaces','/admin/settings','/api/v1/admin/analytics','/api/v1/admin/finance/pnl?from=2026-09-01&to=2026-09-30','/api/v1/admin/finance/supplier-account','/api/v1/admin/procurement/orders/18/saby-prices.xlsx']) expect((await api.get(path)).status()).toBe(403);
  ...
  for(const width of [1440,390]) {
   ...
   await page.goto('/admin?section=returns');await expect(page.getByRole('heading',{name:'Возвраты растений',exact:true})).toBeVisible();await expect(page.getByText('Нужна финансовая сверка',{exact:true})).toBeVisible();await page.getByRole('navigation',{name:'Состояния возвратов'}).getByRole('button',{name:/^Готово к продаже/}).click();await expect(page.getByText('Продажа связана',{exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Создать черновик поступления',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Принять возврат',exact:false})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(1);await page.screenshot({path:`${out}/${role}-stage09-returns-${width}.png`,fullPage:true});
   ...
   if(role==='owner') {
    ...
    await page.goto('/admin?section=finance');await expect(page.getByRole('heading',{name:'Финансы',exact:true})).toBeVisible();await page.getByRole('button',{name:'Прибыль и убытки',exact:true}).click();await expect(page.getByRole('heading',{name:'Прибыль и убытки',exact:true})).toBeVisible();await expect(page.getByText('Предварительный отчёт:',{exact:false})).toBeVisible();await expect(page.getByText('Ожидает сверки',{exact:true})).toHaveCount(2);expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(1);await page.screenshot({path:`${out}/owner-stage11-pnl-${width}.png`,fullPage:true});await page.getByRole('button',{name:'Счёт поставщика',exact:true}).click();await expect(page.getByRole('heading',{name:'Операции',exact:true})).toBeVisible();await expect(page.getByText('Доступный остаток неизвестен',{exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)).toBeLessThanOrEqual(1);await page.screenshot({path:`${out}/owner-stage11-supplier-${width}.png`,fullPage:true});
    ...
    await page.locator('#workspace-content').getByRole('button',{name:'Закупки',exact:true}).click();await page.getByText('CRM-STAGE-06',{exact:true}).click();const invoiceDialog=page.getByRole('dialog',{name:'CRM-STAGE-06',exact:true});await expect(invoiceDialog).toBeVisible();await expect(invoiceDialog.getByText('Совпадает',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('Изменено',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('Нет в инвойсе',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText(/Несопоставленные строки инвойса:/)).toBeVisible();await expect(invoiceDialog.getByRole('button',{name:'Это новая позиция',exact:true})).toBeVisible();await expect(invoiceDialog.getByText('NL-V-09',{exact:false})).toBeVisible();await page.screenshot({path:`${out}/owner-stage06-reconciliation-${width}.png`,fullPage:true});await invoiceDialog.getByRole('button',{name:'Закрыть',exact:true}).click();
    await page.getByText('CRM-STAGE-07',{exact:true}).click();const costDialog=page.getByRole('dialog',{name:'CRM-STAGE-07',exact:true});await expect(costDialog).toBeVisible();await expect(costDialog.getByText('Учётная себестоимость: 945 ₽ · оценочная',{exact:true})).toBeVisible();await expect(costDialog.getByText('Черновик создан · ждёт проведения',{exact:true})).toBeVisible();await expect(costDialog.getByRole('button',{name:'Проведение подтверждено — закрыть',exact:true})).toHaveCount(0);await page.screenshot({path:`${out}/owner-stage07-cost-receipt-${width}.png`,fullPage:true});await costDialog.getByRole('button',{name:'Закрыть',exact:true}).click();
