 ...
 for (const role of ['owner','manager']) {
  ...
  for(const width of [1440,390]) {
   ...
   if(role==='owner') {
    ...
    await page.getByRole('button',{name:'Закупки',exact:true}).click();await page.getByText('CRM-STAGE-06',{exact:true}).click();const invoiceDialog=page.getByRole('dialog',{name:'CRM-STAGE-06',exact:true});await expect(invoiceDialog).toBeVisible();await expect(invoiceDialog.getByText('Совпадает',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('Изменено',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('Нет в инвойсе',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('Добавлено поставщиком',{exact:true})).toBeVisible();await expect(invoiceDialog.getByText('NL-V-09',{exact:false})).toBeVisible();await page.screenshot({path:`${out}/owner-stage06-reconciliation-${width}.png`,fullPage:true});await invoiceDialog.getByRole('button',{name:'Закрыть',exact:true}).click();
