...
SELECT orders.id,product.id,variant.id,variant.sku,'Антуриум Stage 07',variant.label,'{}',2290,1,0,1 FROM orders,products product JOIN product_variants variant ON variant.product_id=product.id WHERE orders.order_number='CRM-STAGE-08' AND product.slug='crm-stage07-a'
UNION ALL SELECT orders.id,product.id,variant.id,variant.sku,'Фикус Stage 07',variant.label,'{}',1790,2,1,0 FROM orders,products product JOIN product_variants variant ON variant.product_id=product.id WHERE orders.order_number='CRM-STAGE-08' AND product.slug='crm-stage07-b';
