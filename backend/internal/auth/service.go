...
// CodeSender delivers a one-time verification code to a phone number,
// e.g. over SMS.
...
// RequestCode sends a one-time login/registration code to the given
// (already normalized) phone number.
func (service *Service) RequestCode(ctx context.Context, phone string) error {
		if phone == "" {
		...
		if err := service.codeSender.Send(ctx, phone, code); err != nil {
