...
func NewRouter(logger *slog.Logger, dependencies Dependencies) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/admin/procurement/orders/{id}/receiving.pdf", procurementAPI.receivingPDF)
	...
	mux.HandleFunc("DELETE /api/v1/admin/procurement/orders/{id}", procurementAPI.deleteOrder)
	...
	mux.HandleFunc("PUT /api/v1/admin/procurement/sales/ignore", procurementAPI.ignoreSales)
	...
	if dependencies.StaticDir != "" {
		// Настройки магазина умеют больше, чем требует этот интерфейс, поэтому
