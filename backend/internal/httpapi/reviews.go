...
type reviewStore interface { Create(context.Context,int64,string,reviews.Input)(int64,error); Photo(context.Context,int64)(string,[]byte,error); ModerationMedia(context.Context,int64)(string,[]byte,error); Pending(context.Context)([]reviews.ModerationItem,error); Moderate(context.Context,int64,int64,string)error; Mine(context.Context,int64)([]reviews.AccountItem,error); UpdateMine(context.Context,int64,int64,int,string)error }
...
func accountReviewsHandler(authentication authService,store reviewStore) http.HandlerFunc{return func(w http.ResponseWriter,r *http.Request){cookie,err:=r.Cookie(auth.CookieName);if err!=nil{writeJSON(w,http.StatusUnauthorized,errorResponse{Error:"Войдите в аккаунт"});return};user,err:=authentication.UserByToken(r.Context(),cookie.Value);if err!=nil||user==nil{writeJSON(w,http.StatusUnauthorized,errorResponse{Error:"Войдите в аккаунт"});return};items,err:=store.Mine(r.Context(),user.ID);if err!=nil{writeJSON(w,http.StatusInternalServerError,errorResponse{Error:"Не удалось загрузить отзывы"});return};writeJSON(w,http.StatusOK,map[string]any{"reviews":items})}}
func updateAccountReviewHandler(authentication authService,store reviewStore) http.HandlerFunc{return func(w http.ResponseWriter,r *http.Request){cookie,err:=r.Cookie(auth.CookieName);if err!=nil{writeJSON(w,http.StatusUnauthorized,errorResponse{Error:"Войдите в аккаунт"});return};user,err:=authentication.UserByToken(r.Context(),cookie.Value);if err!=nil||user==nil{writeJSON(w,http.StatusUnauthorized,errorResponse{Error:"Войдите в аккаунт"});return};id,err:=strconv.ParseInt(r.PathValue("id"),10,64);var body struct{Rating int `json:"rating"`;Text string `json:"text"`};if err!=nil||decodeJSON(r,&body)!=nil{writeJSON(w,http.StatusBadRequest,errorResponse{Error:"Некорректные данные"});return};if err=store.UpdateMine(r.Context(),user.ID,id,body.Rating,body.Text);err!=nil{writeJSON(w,http.StatusConflict,errorResponse{Error:"Не удалось изменить отзыв"});return};writeJSON(w,http.StatusOK,map[string]any{"ok":true})}}
