...
func (store *PostgresStore) ListProducts(ctx context.Context, supplierID int64, query string) ([]ProductDirectoryItem, error) {
	rows, err := store.pool.Query(ctx, `
		...
		SELECT COALESCE(directory.variant_id,0), n.saby_id,
			...
			COALESCE(pc.wb_nm_id, CASE WHEN COALESCE(directory.wb_nm_ids[1], '') ~ '^[0-9]+
		...
		LEFT JOIN LATERAL (
			SELECT o.id AS order_id,COALESCE(o.order_number,'') AS order_number,
				MAX(l.unit_cost_rub)::DOUBLE PRECISION AS unit_cost,
				MAX(l.proposed_marketplace_rub)::DOUBLE PRECISION AS marketplace,
			...
			GROUP BY o.id,o.order_number,o.calculated_at,o.created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.VariantID, &item.SabyID, &item.SabyCode, &item.SabyArticle, &item.Name, &item.SabySection,
			...
			&item.LatestOrderID, &item.LatestOrderNumber, &item.LatestUnitCostRUB,
			&item.SuggestedMarketplaceRUB, &item.SuggestedMarketplaceStrikeRUB); err != nil {
...
func (store *PostgresStore) UpdateProduct(ctx context.Context, actor Actor, input ProductDirectoryUpdate) (ProductDirectoryItem, error) {
	...
	_, err = tx.Exec(ctx, `
		...
		ON CONFLICT (saby_id) DO UPDATE SET holland_article = EXCLUDED.holland_article,
			wb_nm_id = COALESCE(EXCLUDED.wb_nm_id, procurement_product_channels.wb_nm_id),
...
func (store *PostgresStore) LinkChannelProducts(
 ...
 THEN directory.wb_nm_ids[1]::BIGINT END),
