...
type Input struct { Name string `json:"name"`; SabyCode string `json:"sabyCode"`; Category string `json:"category"`; IsPlant bool `json:"isPlant"`; CurrentDescription string `json:"currentDescription"`; Attributes []Attribute `json:"allowedAttributes"` }
...
func(c *Client)Generate(ctx context.Context,input Input,mode string)(Proposal,error){
 ...
 if mode=="care"&&!input.IsPlant{return Proposal{},fmt.Errorf("care generation is only available for plants")}
 task:=map[string]string{"description":"Исправь название и напиши короткое и полное продающее описание.","attributes":"Заполни только разрешённые категорийные характеристики.","care":"Напиши подробную инструкцию по уходу, структурированный паспорт, FAQ и важные предупреждения.","full":"Подготовь всю карточку: аккуратное название, описание и разрешённые характеристики."}[mode];if task==""{return Proposal{},fmt.Errorf("unknown AI mode %q",mode)}
 if input.IsPlant && mode=="description" { task="Исправь название, определи латинское название и напиши короткое и полное продающее описание растения." }
 prompt:=`Ты редактор российского магазина растений и товаров для ухода «Фикусин». `+task+` Не выдумывай разновидность, состав, материал, бренд или модель. Сохраняй бренд и модель у кашпо, грунтов, удобрений и аксессуаров. Если точная идентификация сомнительна, сохрани исходное название и оставь спорные поля пустыми. Используй спокойный русский язык без медицинских и недоказанных обещаний. Для enum используй только перечисленные options.`
 if input.IsPlant { prompt += ` Удали из названия растения технический диаметр D12/Ø12 и размер упаковки, если они уже являются вариантом товара. Не выдумывай сорт по размеру или коду.` }
 prompt += "\nДанные:\n"+string(raw)
 ...
 allProperties:=map[string]any{"name":map[string]any{"type":"string"},"latinName":map[string]any{"type":"string"},"shortDescription":map[string]any{"type":"string"},"description":map[string]any{"type":"string"},"careInstructions":map[string]any{"type":"string"},"attributes":map[string]any{"type":"object","additionalProperties":true},"passport":map[string]any{"type":"object","additionalProperties":false,"properties":passportProperties,"required":passportRequired},"warnings":map[string]any{"type":"array","items":map[string]any{"type":"string"}},"coverPrompt":map[string]any{"type":"string"}}
 descriptionFields:=[]string{"name","shortDescription","description"};if input.IsPlant{descriptionFields=[]string{"name","latinName","shortDescription","description"}}
 fields:=map[string][]string{"description":descriptionFields,"attributes":{"attributes"},"care":{"careInstructions","passport","warnings"},"full":descriptionFields}[mode];properties:=map[string]any{};for _,field:=range fields{properties[field]=allProperties[field]};schema:=map[string]any{"type":"object","additionalProperties":false,"properties":properties,"required":fields}
