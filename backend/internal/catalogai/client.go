...
func(c *Client)Generate(ctx context.Context,input Input,mode string)(Proposal,error){
 ...
 task:=map[string]string{"description":"Исправь название, определи латинское название и напиши короткое и полное продающее описание.","attributes":"Заполни только разрешённые категорийные характеристики.","care":"Напиши подробную инструкцию по уходу, структурированный паспорт, FAQ и важные предупреждения.","full":"Подготовь всю карточку: аккуратное название, описание, разрешённые характеристики и обложку. Для растений также подготовь уход, паспорт, FAQ и предупреждения; для остальных категорий поля ухода оставь пустыми."}[mode];if task==""{return Proposal{},fmt.Errorf("unknown AI mode %q",mode)}
 prompt:=`Ты редактор российского магазина растений и товаров для ухода «Фикусин». `+task+` Удали из названия растения технический диаметр D12/Ø12 и размер упаковки, если они уже являются вариантом товара. Не выдумывай сорт по размеру или коду. Сохраняй бренд и модель у кашпо, грунтов, удобрений и аксессуаров. Если точная идентификация сомнительна, сохрани исходное название и оставь спорные поля пустыми. Используй спокойный русский язык без медицинских и недоказанных обещаний. Для enum используй только перечисленные options. CoverPrompt должен описывать один товар целиком, по центру, на тёплом светло-бежевом фоне, с мягкой естественной тенью, без текста, логотипов, ценников и посторонних предметов; внешний вид товара должен соответствовать названию.`+"\nДанные:\n"+string(raw)
 ...
 fields:=map[string][]string{"description":{"name","latinName","shortDescription","description"},"attributes":{"attributes"},"care":{"careInstructions","passport","warnings"},"full":{"name","latinName","shortDescription","description","careInstructions","attributes","passport","warnings","coverPrompt"}}[mode];properties:=map[string]any{};for _,field:=range fields{properties[field]=allProperties[field]};schema:=map[string]any{"type":"object","additionalProperties":false,"properties":properties,"required":fields}
