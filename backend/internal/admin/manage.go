...
func (repository *PostgresRepository) ListCustomers(ctx context.Context) ([]Customer, error) {
	rows, err := repository.pool.Query(ctx, `
		...
		LEFT JOIN LATERAL (
			...
			WHERE is_active = TRUE AND (customer_id = c.id OR (
				customer_id IS NULL AND c.email IS NOT NULL AND LOWER(email) = LOWER(c.email)
			)) ORDER BY (customer_id = c.id) DESC LIMIT 1
		) au ON TRUE
