...
type OrderEditLine struct {
	SKU       string `json:"sku"`
	// ProductID is accepted temporarily for requests from an older admin bundle.
	// In that compatibility path it contains the old string identifier, which
	// Catalog v2 converted to the SKU. New clients send SKU explicitly.
...
func orderEditSKU(line OrderEditLine) string {
...
func normalizeOrderEditLines(lines []OrderEditLine) ([]OrderEditLine, error) {
	...
	for _, line := range lines {
		...
		if index, ok := positions[sku]; ok {
			...
			if result[index].Quantity > 100 {
				return nil, fmt.Errorf("слишком большое количество товара")
			}
			continue
		}
	...
	if len(result) == 0 {
		return nil, fmt.Errorf("заказ не может быть пустым — отмените его")
	}
	return result, nil
}
...
func releaseOrderReservationsForEdit(ctx context.Context, tx pgx.Tx, orderID int64) error {
	...
	`, orderID); err != nil {
		return fmt.Errorf("lock inventory for order edit: %w", err)
	}
	if _, err := tx.Exec(ctx, `
		WITH wanted AS (
	...
	`, orderID); err != nil {
		return fmt.Errorf("release inventory for order edit: %w", err)
	}
	return nil
}
...
func (repository *PostgresRepository) EditOrder(ctx context.Context, actor Actor, id int64, edit OrderEdit) (Order, error) {
	...
	if edit.DeliveryFee != nil && (*edit.DeliveryFee < 0 || *edit.DeliveryFee > 100000) {
	...
	var oldDeliveryFee float64
	var oldFeePending bool
	if err := tx.QueryRow(ctx, `SELECT status,delivery_method,customer_id,delivery_fee::DOUBLE PRECISION,delivery_fee_pending=1 FROM orders WHERE id=$1 FOR UPDATE`, id).Scan(&status, &deliveryMethod, &customerID, &oldDeliveryFee, &oldFeePending); err != nil {
		return Order{}, err
	}
	if status == "cancelled" || status == "completed" || status == "shipped" {
		return Order{}, fmt.Errorf("состав уже закрытого или отправленного заказа менять нельзя")
	}

	var before map[string]any
	if err := tx.QueryRow(ctx, `SELECT jsonb_build_object('subtotal',subtotal,'deliveryFee',delivery_fee,'total',total,'hasPreorder',has_preorder,'feePending',delivery_fee_pending) FROM orders WHERE id=$1`, id).Scan(&before); err != nil {
		return Order{}, err
	}

	if edit.Items != nil {
		lines, err := normalizeOrderEditLines(*edit.Items)
		if err != nil {
			return Order{}, err
		}
		oldPrices := map[string]float64{}
		rows, err := tx.Query(ctx, `SELECT sku,unit_price::DOUBLE PRECISION FROM order_items WHERE order_id=$1`, id)
		if err != nil {
			return Order{}, err
		}
		for rows.Next() {
		...
		if err := releaseOrderReservationsForEdit(ctx, tx, id); err != nil {
			return Order{}, err
		}
		if _, err := tx.Exec(ctx, `UPDATE procurement_requests SET status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE customer_order_id=$1 AND kind='customer_order' AND status='open'`, id); err != nil {
			return Order{}, err
		}
		...
		discountBPS := 0
		if customerID != nil {
			_ = tx.QueryRow(ctx, `SELECT COALESCE(retail_discount_bps,0) FROM customers WHERE id=$1`, *customerID).Scan(&discountBPS)
		}
		hasPreorder := false
		for _, requested := range lines {
			var line editableOrderLine
			var priceMinor int64
			if err := tx.QueryRow(ctx, `
				SELECT pv.sku,p.id,pv.id,p.name,pv.label,pv.height_cm,pv.pot_diameter_cm,
					pv.base_price_minor,COALESCE(p.saby_id,'')
				FROM products p
				...
				LIMIT 1
			`, requested.SKU).Scan(
				&line.sku, &line.productID, &line.variantID, &line.name, &line.variantLabel,
				&line.heightCM, &line.potDiameterCM, &priceMinor, &line.sabyID,
			); err != nil {
				if errors.Is(err, pgx.ErrNoRows) {
					return Order{}, fmt.Errorf("SKU %s больше не доступен", requested.SKU)
				}
				return Order{}, err
			}
			line.quantity = requested.Quantity
			if price, ok := oldPrices[line.sku]; ok {
				line.price = price
			} else {
				minor := priceMinor
				if discountBPS > 0 {
					...
					minor = (priceMinor*int64(10000-discountBPS) + 5000) / 10000
				}
				line.price = float64(minor) / 100
			}
			line.reserved, line.preorder, err = reserveVariantForEdit(ctx, tx, line.variantID, line.quantity)
			if err != nil {
				return Order{}, err
			}
			hasPreorder = hasPreorder || line.preorder
			if _, err := tx.Exec(ctx, `
			...
			`, id, line.productID, line.variantID, line.sku, line.name, line.variantLabel,
				line.heightCM, line.potDiameterCM, line.price, line.quantity,
				boolToSmallInt(line.preorder), line.reserved); err != nil {
				return Order{}, err
			}
			if line.preorder {
				if _, err := tx.Exec(ctx, `INSERT INTO procurement_requests(kind,saby_id,requested_name,quantity,customer_order_id,status,notes) VALUES('customer_order',NULLIF($1,''),$2,$3,$4,'open','Предзаказ после изменения заказа')`, line.sabyID, line.name, line.quantity, id); err != nil {
					return Order{}, err
				}
			}
		}
		if err := order.RecordMovement(ctx, tx, id, order.MovementReserve); err != nil {
			return Order{}, err
		}
		...
		if _, err := tx.Exec(ctx, `UPDATE orders SET has_preorder=$2,delivery_fee_pending=$3,stock_released_at=NULL WHERE id=$1`, id, boolToSmallInt(hasPreorder), boolToSmallInt(oldFeePending)); err != nil {
			return Order{}, err
		}
	}

	if edit.DeliveryFee != nil {
		if _, err := tx.Exec(ctx, `UPDATE orders SET delivery_fee=$2,delivery_fee_pending=0,delivery_repack_requested=0 WHERE id=$1`, id, *edit.DeliveryFee); err != nil {
			return Order{}, err
		}
	}
	if _, err := tx.Exec(ctx, `
	...
	if err := tx.QueryRow(ctx, `SELECT jsonb_build_object('subtotal',subtotal,'deliveryFee',delivery_fee,'total',total,'hasPreorder',has_preorder,'feePending',delivery_fee_pending) FROM orders WHERE id=$1`, id).Scan(&after); err != nil {
		return Order{}, err
	}
	if err := insertAudit(ctx, tx, actor, "order.contents.update", "order", fmt.Sprint(id), before, after); err != nil {
		return Order{}, err
	}
