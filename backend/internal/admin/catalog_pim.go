...
func (repository *PostgresRepository) ListProductVariants(ctx context.Context, productID int64) ([]AdminVariant,error) {
	rows,err:=repository.pool.Query(ctx,`
		SELECT v.id,v.product_id,v.sku,v.label,v.base_price_minor,
			...
			COALESCE((SELECT jsonb_agg(COALESCE(mirror.large_url,m.object_key) ORDER BY m.is_primary DESC,m.sort_order,m.id) FROM product_media m LEFT JOIN media_mirror mirror ON mirror.source_url=m.object_key WHERE m.variant_id=v.id),'[]'::jsonb)
		FROM product_variants v WHERE v.product_id=$1 ORDER BY v.archived_at NULLS FIRST,v.id
	`,productID);if err!=nil{return nil,fmt.Errorf("list variants: %w",err)};defer rows.Close();items:=[]AdminVariant{}
	for rows.Next(){var item AdminVariant;var priceMinor int64;var attributes,external,images []byte;if err:=rows.Scan(&item.ID,&item.ProductID,&item.SKU,&item.Label,&priceMinor,&item.Stock,&item.WholesaleMinQty,&item.Active,&item.Archived,&attributes,&external,&images);err!=nil{return nil,err};item.Price=float64(priceMinor)/100;item.Attributes=decodeJSONMap(attributes);item.ExternalIDs=decodeExternalIDs(external);item.Images=decodeStrings(images);items=append(items,item)};return items,rows.Err()
}
