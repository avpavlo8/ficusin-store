...
func (repository *PostgresRepository) CreateProductVariant(ctx context.Context, actor Actor, productID int64, input VariantInput) (AdminVariant,error) {
	if !Can(actor.Role,PermissionProductsManage){return AdminVariant{},ErrForbidden};if err:=validateVariantInput(input);err!=nil{return AdminVariant{},err};tx,err:=repository.pool.Begin(ctx);if err!=nil{return AdminVariant{},err};defer func(){_ = tx.Rollback(ctx)}()
