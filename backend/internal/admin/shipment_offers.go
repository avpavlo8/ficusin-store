...
func (repository *PostgresRepository) CreateShipmentOffer(ctx context.Context,actor Actor,orderID int64,input ShipmentOfferInput)(ShipmentOffer,error){
	...
	var revision int;var status,delivery,address,email string
	if err:=tx.QueryRow(ctx,`SELECT shipment_revision,status,delivery_method,address,COALESCE(email,'') FROM orders WHERE id=$1 FOR UPDATE`,orderID).Scan(&revision,&status,&delivery,&address,&email);err!=nil{return ShipmentOffer{},err}
	...
	for _,requested:=range input.Items{
		...
		var variantID *int64;var sku,name string;var price float64;var ordered,already int
		if err:=tx.QueryRow(ctx,`SELECT oi.variant_id,COALESCE(oi.sku,''),oi.product_name,oi.unit_price::DOUBLE PRECISION,oi.quantity,COALESCE((SELECT SUM(soi.quantity) FROM shipment_offer_items soi JOIN shipment_offers so ON so.id=soi.shipment_offer_id WHERE soi.order_item_id=oi.id AND so.status='paid'),0)::INTEGER FROM order_items oi WHERE oi.id=$1 AND oi.order_id=$2`,requested.OrderItemID,orderID).Scan(&variantID,&sku,&name,&price,&ordered,&already);err!=nil{return ShipmentOffer{},errors.New("позиция заказа не найдена")}
		...
		if _,err:=tx.Exec(ctx,`INSERT INTO shipment_offer_items(shipment_offer_id,order_item_id,variant_id,sku,product_name,unit_price,quantity) VALUES($1,$2,$3,$4,$5,$6,$7)`,offerID,requested.OrderItemID,variantID,sku,name,price,requested.Quantity);err!=nil{return ShipmentOffer{},err};selected[requested.OrderItemID]=requested.Quantity;subtotal+=price*float64(requested.Quantity)
	}
...
func (repository *PostgresRepository) ShipmentOffer(ctx context.Context,id int64)(ShipmentOffer,error){
	...
	items,err:=repository.pool.Query(ctx,`SELECT order_item_id,sku,product_name,unit_price::DOUBLE PRECISION,quantity FROM shipment_offer_items WHERE shipment_offer_id=$1 ORDER BY id`,id);if err!=nil{return result,err};defer items.Close();result.Items=[]ShipmentOfferItem{};for items.Next(){var item ShipmentOfferItem;if err:=items.Scan(&item.OrderItemID,&item.SKU,&item.ProductName,&item.UnitPrice,&item.Quantity);err!=nil{return result,err};result.Items=append(result.Items,item)};if err:=items.Err();err!=nil{return result,err}
