...
func TestRussianPostCalculateUsesContractRateAndDimensions(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/1.0/tariff":
			if err := json.NewDecoder(request.Body).Decode(&tariffBody); err != nil { t.Fatal(err) }
			_ = json.NewEncoder(response).Encode(map[string]any{
				"total-rate": 50000,
				"total-vat":  10000,
	...
	if quote.Price != 600 || quote.DaysMin != 2 || quote.DaysMax != 4 { t.Fatalf("неверный тариф: %+v", quote) }
	if tariffBody["index-from"] != "390000" || tariffBody["index-to"] != "101000" { t.Fatalf("индексы потерялись: %#v", tariffBody) }
	dimension, ok := tariffBody["dimension"].(map[string]any)
	if !ok { t.Fatalf("нет габаритов: %#v", tariffBody["dimension"]) }
...
func TestRussianPostUnrecognizedAddressWaitsForManager(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		_ = json.NewEncoder(response).Encode([]map[string]any{{"index": "", "quality-code": "UNDEF_05"}})
	}))
	defer server.Close()
	client := NewRussianPostClient("token", "key", "390000")
	client.baseURL = server.URL
	client.httpClient = server.Client()
	quote, err := client.Calculate(context.Background(), "непонятный адрес", Parcel{
		LengthCM: 10, WidthCM: 10, HeightCM: 10, WeightGrams: 100,
	})
	if err != nil { t.Fatalf("неизвестный адрес должен уйти менеджеру, а не сломать заказ: %v", err) }
	if quote.Price != 0 { t.Fatalf("по неизвестному адресу нельзя придумывать стоимость: %+v", quote) }
}

func TestRussianPostNeedsRealCredentials(t *testing.T) {
	if NewRussianPostClient("", "", "").Configured() { t.Fatal("пустые ключи включили Почту России") }
	if !NewRussianPostClient("token", "key", "390000").Configured() { t.Fatal("полный набор ключей не включил Почту России") }
}
