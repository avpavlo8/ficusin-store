...
func (repository *PostgresRepository) DetailForCustomer(
	...
	for offerRows.Next(){var offer ShipmentOffer;if err:=offerRows.Scan(&offer.ID,&offer.PaymentToken,&offer.Status,&offer.DeliveryFee,&offer.Subtotal,&offer.Total,&offer.NotifiedAt,&offer.ExpiresAt,&offer.Boxes);err!=nil{offerRows.Close();return nil,err};items,err:=repository.pool.Query(ctx,`SELECT product_name,unit_price::DOUBLE PRECISION,quantity FROM shipment_offer_items WHERE shipment_offer_id=$1 ORDER BY id`,offer.ID);if err!=nil{offerRows.Close();return nil,err};offer.Items=[]Item{};for items.Next(){var item Item;if err:=items.Scan(&item.ProductName,&item.UnitPrice,&item.Quantity);err!=nil{items.Close();offerRows.Close();return nil,err};offer.Items=append(offer.Items,item)};items.Close();if offer.Status!="offered"&&offer.Status!="payment_pending"{offer.PaymentToken=""};detail.ShipmentOffers=append(detail.ShipmentOffers,offer)}
