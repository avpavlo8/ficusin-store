...
func (worker *ShippingWorker) createOfferShipments(ctx context.Context) {
	...
	if err != nil { worker.logger.Error("find shipment offers to ship failed", "error", err); return }
	type pending struct { offerID,orderID int64; number,name,phone,office string; tariff,city,attempts int }
	waiting:=make([]pending,0)
	for rows.Next(){var item pending;if err:=rows.Scan(&item.offerID,&item.orderID,&item.number,&item.name,&item.phone,&item.office,&item.tariff,&item.city,&item.attempts);err!=nil{worker.logger.Error("scan shipment offer failed","error",err);break};waiting=append(waiting,item)}
	rows.Close()
	for _,item:=range waiting{
		packages,err:=worker.offerPackages(ctx,item.offerID,item.number)
		if err==nil&&len(packages)==0{err=errors.New("у предложения нет коробок")}
		if err!=nil{worker.recordOfferCreateFailure(ctx,item.offerID,item.number,item.attempts,err);continue}
		shipment,err:=worker.cdek.CreateOrder(ctx,integration.ShipmentRequest{OrderNumber:fmt.Sprintf("%s-%d",item.number,item.offerID),TariffCode:item.tariff,OfficeCode:item.office,CityCode:item.city,Packages:packages,SenderName:worker.settings.Value(settings.CDEKSenderName),SenderPhone:worker.settings.Value(settings.CDEKSenderPhone),SenderAddress:worker.settings.Value(settings.CDEKSenderAddress),RecipientName:item.name,RecipientPhone:item.phone})
		if err!=nil{worker.recordOfferCreateFailure(ctx,item.offerID,item.number,item.attempts,err);continue}
		if _,err:=worker.pool.Exec(ctx,`UPDATE shipment_offers SET cdek_uuid=$2,cdek_create_state='active',cdek_last_error='',cdek_next_attempt_at=NULL,cdek_synced_at=CURRENT_TIMESTAMP,status='shipped',updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND cdek_uuid=''`,item.offerID,shipment.UUID);err!=nil{worker.logger.Error("store offer cdek uuid failed","error",err,"order",item.number)}
	}
}
...
func (worker *ShippingWorker) recordOfferCreateFailure(ctx context.Context,offerID int64,number string,attempts int,failure error){delay:=time.Minute*time.Duration(1<<min(6,max(0,attempts-1)));message:=failure.Error();if len(message)>1000{message=message[:1000]};if _,err:=worker.pool.Exec(ctx,`UPDATE shipment_offers SET status='paid',cdek_create_state='retry',cdek_last_error=$2,cdek_next_attempt_at=CURRENT_TIMESTAMP+($3*INTERVAL '1 second'),updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND cdek_uuid=''`,offerID,message,int(delay/time.Second));err!=nil{worker.logger.Error("store offer cdek failure failed","error",err,"order",number)}}
