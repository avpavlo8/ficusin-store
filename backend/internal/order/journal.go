...
const recordMovementSQL = `
		INSERT INTO stock_movements (order_id, variant_id, saby_id, kind, quantity, status)
		SELECT oi.order_id, oi.variant_id, COALESCE(pv.saby_id, ''), $2,
			SUM(CASE WHEN $2 IN ('reserve', 'release') THEN oi.reserved_qty ELSE oi.quantity END)::INTEGER,
			'pending'
		FROM order_items oi
		LEFT JOIN product_variants pv ON pv.id = oi.variant_id
		WHERE oi.order_id = $1 AND oi.variant_id IS NOT NULL
			AND (CASE WHEN $2 IN ('reserve', 'release') THEN oi.reserved_qty ELSE oi.quantity END) > 0
...
func RecordMovement(ctx context.Context, tx pgx.Tx, orderID int64, kind string) error {
	if _, err := tx.Exec(ctx, recordMovementSQL, orderID, kind); err != nil {
