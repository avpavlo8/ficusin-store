  ...
  return <section className="admin-block procurement-block">
    ...
    <div className="procurement-catalog-mirror">
      ...
      <div className="procurement-mirror-content">
        ...
        {visibleItems.length ? <div className="admin-table-wrap"><table className="admin-table procurement-directory"><thead><tr><th>Товар Saby</th><th>СБИС</th><th>Цена МП</th><th>Сайт</th><th>WB / продажи</th><th>Ozon / продажи</th><th>Поставщик / закупка</th><th></th></tr></thead><tbody>{visibleItems.map((item) => <tr key={`${item.supplierId}-${item.sabyId}`}>
          ...
          <td>{item.suggestedMarketplaceRub != null ? <><strong>{item.suggestedMarketplaceRub.toLocaleString("ru-RU")} ₽</strong>{item.suggestedMarketplaceStrikeRub != null && item.suggestedMarketplaceStrikeRub > item.suggestedMarketplaceRub && <small>До скидки: {item.suggestedMarketplaceStrikeRub.toLocaleString("ru-RU")} ₽</small>}{item.unitCostRub != null && <small>Себестоимость: {item.unitCostRub.toLocaleString("ru-RU")} ₽</small>}<small>{item.priceSourceOrderNumber ? `По закупке ${item.priceSourceOrderNumber}` : "Последний расчёт закупки"}</small></> : <><strong>Нет расчёта</strong><small>Сначала рассчитайте закупку с этим товаром</small></>}</td>
          ...
          <td>{(item.wbArticles || []).map((article) => <small key={`wb-${article}`}>{article}</small>)}{!(item.wbArticles || []).length && <small>Не связан</small>}{item.wbNmId && <small>nmID: {item.wbNmId}</small>}<strong>{item.wbSales} продаж</strong>{(item.wbLegacyArticles || []).map((article) => <small key={`wb-legacy-${article}`}>Архивный: {article}</small>)}</td>
          ...
          <td><strong>{item.supplierName}</strong><small>{item.supplierArticle || item.hollandArticle || "Артикул поставщика не заполнен"}</small><small>{availabilityLabel(item.availabilityStatus)}</small><small>{item.supplierCategory || item.aliases[0] || "Закупок пока нет"}{item.expectedUnitPrice ? ` · ${item.expectedUnitPrice.toFixed(2)} €` : ""}</small></td>
