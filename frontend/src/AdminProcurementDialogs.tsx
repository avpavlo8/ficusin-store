...
export function ProcurementOrderDetailDialog({ orderId, onClose, onSaved, onError }: { orderId: number; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  const [priceChannels, setPriceChannels] = useState<Record<string, boolean>>({ saby_price: true, site: false, wb: false, ozon: false });
  ...
  const prepare = async (kind: "receipt" | "prices") => { const channels = kind === "prices" ? Object.entries(priceChannels).filter(([, selected]) => selected).map(([channel]) => channel) : []; setSaving(true); try { await api(`/api/v1/admin/procurement/orders/${orderId}/batches`, { method: "POST", body: JSON.stringify({ kind, channels }) }); await load(); } catch (error) { onError((error as Error).message); } finally { setSaving(false); } };
  const approve = async (batch: ProcurementActionBatch) => { if (!window.confirm(batch.kind === "prices" ? "Подтвердить этот список и подготовить изменения только в выбранных каналах?" : "Подтвердить состав черновика поступления?")) return; setSaving(true); try { await api(`/api/v1/admin/procurement/batches/${batch.id}/approve`, { method: "POST" }); await load(); onSaved(); } catch (error) { onError((error as Error).message); } finally { setSaving(false); } };
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog procurement-order-dialog" role="dialog" aria-modal="true" aria-labelledby="order-detail-title"><header><div><p className="eyebrow">Закупка</p><h2 id="order-detail-title">{detail?.order.orderNumber || `№${orderId}`}</h2></div><button onClick={onClose} aria-label="Закрыть">×</button></header>
    {!detail ? <div className="procurement-zero">Загружаем строки…</div> : <div className="procurement-order-body">
      ...
      {detail.order.status === "ready_to_receive" && <><fieldset className="procurement-price-channels"><legend>Где подготовить изменение цен</legend>{[["saby_price", "СБИС"], ["site", "Сайт"], ["wb", "Wildberries"], ["ozon", "Ozon"]].map(([channel, label]) => <label key={channel}><input type="checkbox" checked={priceChannels[channel]} onChange={(event) => setPriceChannels({ ...priceChannels, [channel]: event.target.checked })} />{label}</label>)}</fieldset><div className="procurement-batch-buttons"><button onClick={() => void setStatus("review")} disabled={saving}>Вернуть на проверку</button><button onClick={() => void prepare("receipt")} disabled={saving || !detail.validation.canPrepareActions}>Подготовить поступление СБИС</button><button className="admin-primary" onClick={() => void prepare("prices")} disabled={saving || !detail.validation.canPrepareActions || !Object.values(priceChannels).some(Boolean)}>Подготовить изменение цен</button></div></>}
      {detail.batches.map((batch) => <section className="procurement-batch" key={batch.id}><div className="admin-block-heading"><div><p className="eyebrow">{batch.kind === "prices" ? "Цены" : "Поступление"}</p><h3>{batch.kind === "prices" ? "Черновик СБИС и изменения по каналам" : "Черновик для СБИС"}</h3></div><span className="admin-pill">{batchStatusLabel(batch.status)}</span></div><div className="admin-table-wrap procurement-batch-table"><table className="admin-table"><thead><tr><th>Товар</th><th>Канал</th><th>Было</th><th>Станет / количество</th><th>Статус</th></tr></thead><tbody>{batchDisplayRows(batch).map((row) => <tr key={row.key}><td><strong>{row.productName}</strong>{row.productCode && <small>Код СБИС: {row.productCode}</small>}</td><td>{channelLabel(row.item.channel)}{row.item.externalArticle && !row.item.previewLines?.length && <small>{row.item.externalArticle}</small>}</td><td>{row.oldValue == null ? "—" : money.format(row.oldValue)}</td><td>{row.quantity ?? money.format(row.newValue)}{row.item.compareAtValue && row.item.compareAtValue > row.newValue ? <small>до скидки {money.format(row.item.compareAtValue)}</small> : null}</td><td className="procurement-action-status">{row.showStatus ? actionStatus(row.item) : null}</td></tr>)}</tbody></table></div>{batch.status === "draft" && <div className="dialog-actions"><button className="primary" disabled={saving || !batch.items.length} onClick={() => void approve(batch)}>{batch.kind === "receipt" ? "Создать черновик в СБИС" : `Подтвердить ${batchDisplayRows(batch).length} строк`}</button></div>}{batch.items.some((item) => item.status === "failed") && <div className="dialog-actions"><button disabled={saving} onClick={() => void retry(batch)}>Повторить ошибки</button></div>}</section>)}
...
const batchDisplayRows = (batch: ProcurementActionBatch) => batch.items.flatMap((item) => item.previewLines?.length
  ? item.previewLines.map((line, index) => ({ key: `${item.id}-${line.sabyId}`, item, productName: line.name, productCode: line.code, oldValue: line.oldPrice, newValue: line.newPrice || 0, quantity: line.quantity, showStatus: index === 0 }))
  : [{ key: String(item.id), item, productName: item.productName, productCode: item.productCode, oldValue: item.oldValue, newValue: item.newValue, quantity: item.quantity, showStatus: true }]);
...
export const actionStatus = (item: ProcurementActionItem) => {
  ...
  return <><span className={item.status === "completed" ? "procurement-ok" : item.status === "failed" || item.status === "not_configured" || item.status === "skipped" ? "procurement-warning" : ""}>{label}</span>{item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noreferrer"><small>Открыть черновик в СБИС</small></a>}{item.errorMessage && <small>{item.errorMessage}</small>}</>;
};
...
export function ProcurementMatchDialog({ alias, onClose, onSaved, onError }: { alias: ProcurementAlias; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog procurement-match-dialog" role="dialog" aria-modal="true" aria-labelledby="match-title"><header><div><p className="eyebrow">{alias.supplierName}</p><h2 id="match-title">Сопоставить товар</h2></div><button onClick={onClose} aria-label="Закрыть">×</button></header>
	...
	<p className="admin-hint">Если карточку только что создали в СБИС, обновите справочник — она появится здесь сразу.</p>
