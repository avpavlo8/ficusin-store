...
export function SupplierDialog({ suppliers, onClose, onSaved, onError }: { suppliers: ProcurementSupplier[]; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  const save = async () => { setSaving(true); try { await api("/api/v1/admin/procurement/suppliers", { method: "POST", body: JSON.stringify({ name, kind, countryCode, taxId, defaultCurrency: currency }) }); setName(""); setTaxId(""); onSaved(); } catch (error) { onError((error as Error).message); } finally { setSaving(false); } };
