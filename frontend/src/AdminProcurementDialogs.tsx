...
export function ProcurementPlanDialog({ suppliers, recommendations, settings, onClose, onSaved, onError }: { suppliers: ProcurementSupplier[]; recommendations: ProcurementRecommendation[]; settings: ProcurementSettings; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  const draftStorageKey = "ficusin:procurement-plan-draft:v1";
	const preferred = suppliers.find((item) => item.id === recommendations[0]?.supplierId) || suppliers.find((item) => item.kind === "international") || suppliers[0];
  ...
  const selectedSupplier = suppliers.find((item) => item.id === supplierId); const isDomestic = selectedSupplier?.kind === "domestic";
  ...
  const isEmptyLine = (item: DraftLine) => !item.sabyId && !item.sabyName.trim() && !item.article.trim() && !item.category.trim() && !item.expectedUnitPrice && !item.potDiameterCm && !item.heightCm;
	const [items, setItems] = useState<DraftLine[]>(() => savedPlan?.items?.length ? savedPlan.items : recommendations.length ? recommendations.map(makeLine) : [makeLine()]);
  ...
  const decimalNumber = (value: NumericDraft) => Number(value.trim().replace(",", "."));
  ...
  const missingFields = (item: DraftLine) => {
    ...
    if (!isDomestic && !(Number(item.heightCm) > 0)) missing.push("высота");
    return missing;
  };
  ...
  const valid = selected.length > 0 && (isDomestic || Number(exchangeRate) > 0) && (isDomestic || Number(deliveryToMoscowRub) >= 0 && Number(deliveryToRyazanRub) >= 0) && selected.every((item) =>
    ...
    decimalNumber(item.expectedUnitPrice) > 0 && (isDomestic || Number(item.potDiameterCm) > 0 && Number(item.heightCm) > 0));
