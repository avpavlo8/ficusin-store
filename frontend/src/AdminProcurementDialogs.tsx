...
export function ProcurementOrderDetailDialog({ orderId, onClose, onSaved, onError }: { orderId: number; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog procurement-order-dialog" role="dialog" aria-modal="true" aria-labelledby="order-detail-title"><header><div><p className="eyebrow">Закупка</p><h2 id="order-detail-title">{detail?.order.orderNumber || `№${orderId}`}</h2></div><button onClick={onClose} aria-label="Закрыть">×</button></header>
    {!detail ? <div className="procurement-zero">Загружаем строки…</div> : <div className="procurement-order-body">
      ...
      {detail.order.status === "ready_to_receive" && <><fieldset className="procurement-price-channels"><legend>Где подготовить изменение цен</legend><label className="disabled" title="Публичный API Saby не поддерживает внутренний документ «Изменение цен»"><input type="checkbox" checked={false} disabled />СБИС — нужен импорт XLSX</label>{[["site", "Сайт"], ["wb", "Wildberries"], ["ozon", "Ozon"]].map(([channel, label]) => <label key={channel}><input type="checkbox" checked={priceChannels[channel]} onChange={(event) => setPriceChannels({ ...priceChannels, [channel]: event.target.checked })} />{label}</label>)}</fieldset><p className="admin-hint procurement-note">Для СБИС больше не отправляется выдуманный тип документа. Поступление создаётся через API; изменение цен Saby официально принимает через импорт XLSX.</p><div className="procurement-batch-buttons"><button onClick={() => void setStatus("review")} disabled={saving}>Вернуть на проверку</button><button onClick={() => void prepare("receipt")} disabled={saving || !detail.validation.canPrepareActions}>Подготовить поступление СБИС</button><button className="admin-primary" onClick={() => void prepare("prices")} disabled={saving || !detail.validation.canPrepareActions || !Object.values(priceChannels).some(Boolean)}>Подготовить изменение цен</button></div></>}
      {detail.batches.map((batch) => <section className="procurement-batch" key={batch.id}><div className="admin-block-heading"><div><p className="eyebrow">{batch.kind === "prices" ? "Цены" : "Поступление"}</p><h3>{batch.kind === "prices" ? "Изменения по выбранным каналам" : "Черновик для СБИС"}</h3></div><span className="admin-pill">{batchStatusLabel(batch.status)}</span></div><div className="procurement-batch-list">{batchDisplayRows(batch).map((row) => <article key={row.key}><div className="procurement-batch-product"><strong>{row.productName}</strong>{row.productCode && <small>Код СБИС: {row.productCode}</small>}</div><div><small>Канал</small><span>{channelLabel(row.item.channel)}</span>{row.item.externalArticle && !row.item.previewLines?.length && <small>{row.item.externalArticle}</small>}</div><div><small>{row.quantity == null ? "Было → станет" : "Количество"}</small><span>{row.quantity ?? `${row.oldValue == null ? "—" : money.format(row.oldValue)} → ${money.format(row.newValue)}`}</span>{row.item.compareAtValue && row.item.compareAtValue > row.newValue ? <small>до скидки {money.format(row.item.compareAtValue)}</small> : null}</div>{row.showStatus && <div className="procurement-action-status">{actionStatus(row.item)}</div>}</article>)}</div>{batch.status === "draft" && <div className="dialog-actions"><button className="primary" disabled={saving || !batch.items.length} onClick={() => void approve(batch)}>{batch.kind === "receipt" ? "Создать черновик в СБИС" : `Подтвердить ${batchDisplayRows(batch).length} строк`}</button></div>}{batch.items.some((item) => item.status === "failed") && <div className="dialog-actions"><button disabled={saving} onClick={() => void retry(batch)}>Повторить ошибки</button></div>}</section>)}
