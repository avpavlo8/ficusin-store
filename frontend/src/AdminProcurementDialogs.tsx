...
export function ProcurementPlanDialog({ suppliers, recommendations, settings, onClose, onSaved, onError }: { suppliers: ProcurementSupplier[]; recommendations: ProcurementRecommendation[]; settings: ProcurementSettings; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  type DraftLine = { id: string; sabyId: string; sabyName: string; rawName: string; article: string; category: string; packageCount: NumericDraft; unitsPerPackage: NumericDraft; expectedUnitPrice: NumericDraft; potDiameterCm: NumericDraft; heightCm: NumericDraft; loadUnit: string };
  const preferred = suppliers.find((item) => item.kind === "international") || suppliers[0];
  const makeLine = (item?: ProcurementRecommendation): DraftLine => ({ id: item?.sabyId || `new-${Date.now()}-${Math.random()}`, sabyId: item?.sabyId || "", sabyName: item?.name || "", rawName: item?.dutchName || "", article: item?.supplierArticle || "", category: "", packageCount: "1", unitsPerPackage: String(item?.orderMultiple || 1), expectedUnitPrice: item?.lastUnitPrice?.toString() || "", potDiameterCm: item?.potDiameterCm?.toString() || "", heightCm: item?.heightCm?.toString() || "", loadUnit: "shelf" });
  ...
  const [items, setItems] = useState<DraftLine[]>([]); const [showRecommendations, setShowRecommendations] = useState(false);
  ...
  const addRecommendation = (recommendation: ProcurementRecommendation) => setItems((current) => [...current, makeLine(recommendation)]);
  const sorted = items;
  const sortItems = () => setItems((current) => [...current].sort((a, b) => a.category.localeCompare(b.category, "ru") || a.rawName.localeCompare(b.rawName, "nl")));
  ...
  const valid = selected.length > 0 && Number(exchangeRate) > 0 && Number(deliveryToMoscowRub) >= 0 && Number(deliveryToRyazanRub) >= 0 && selected.every((item) =>
    (item.sabyId || item.rawName.trim()) && Number.isInteger(Number(item.packageCount)) && Number(item.packageCount) > 0 &&
  ...
  const requestBody = JSON.stringify({ supplierId, orderNumber,
    ...
    items: selected.map((item) => ({ sabyId: item.sabyId, rawName: item.rawName, category: item.category, supplierArticle: item.article,
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog procurement-plan-dialog procurement-plan-fullscreen" role="dialog" aria-modal="true" aria-labelledby="plan-title"><header><div><p className="eyebrow">Закупка в Голландии</p><h2 id="plan-title">Новый заказ поставщику</h2></div><button className="procurement-plan-close" onClick={onClose} aria-label="Закрыть">×</button></header>
    <div className="procurement-plan-settings"><label>Поставщик<select value={supplierId} onChange={(event) => { if (items.length && !window.confirm("Сменить поставщика и очистить текущий список?")) return; setSupplierId(Number(event.target.value)); setItems([]); }}>{suppliers.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Номер заказа<input value={orderNumber} onChange={(event) => setOrderNumber(event.target.value)} placeholder="Можно оставить пустым" /></label><label>Курс валюты<input type="number" min="0" step="0.01" value={exchangeRate} onChange={(event) => setExchangeRate(draftNumber(event.target.value))} /></label><label>Логистика до Москвы, ₽<input type="number" min="0" value={deliveryToMoscowRub} placeholder="0" onChange={(event) => setDeliveryToMoscowRub(draftNumber(event.target.value))} /></label><label>Москва → Рязань, ₽<input type="number" min="0" value={deliveryToRyazanRub} placeholder="0" onChange={(event) => setDeliveryToRyazanRub(draftNumber(event.target.value))} /></label></div>
    <div className="procurement-plan-tools"><button className="admin-primary" onClick={() => setShowRecommendations(true)}>Заполнить из рекомендаций</button><button onClick={() => setItems((current) => [...current, makeLine()])}>+ Новая позиция</button><button onClick={sortItems}>По категории и названию</button><span>{selected.length} позиций · {totalUnits} шт.</span></div>
    {!items.length ? <div className="procurement-plan-empty"><strong>Заказ пока пуст</strong><span>Добавьте позиции из рекомендаций или создайте новую строку вручную.</span></div> : <div className="admin-table-wrap procurement-plan-table-wrap"><table className="admin-table procurement-plan-table"><thead><tr><th>Товар СБИС</th><th>Название в Голландии</th><th>Артикул Голландии</th><th>Горшок / высота</th><th>Упаковок × в упаковке</th><th>Цена, €</th><th>Закупка / себестоимость</th><th>Действие</th></tr></thead><tbody>{sorted.map((item, index) => { const units = Number(item.packageCount) * Number(item.unitsPerPackage); const estimate = valid && preview?.key === requestBody ? preview.lines[index] : undefined; return <tr key={item.id}><td><strong>{item.sabyName || "Новая позиция"}</strong>{<input value={item.category} onChange={(event) => updateItem(item.id, { category: event.target.value })} placeholder="Категория" />}</td><td><input value={item.rawName} onChange={(event) => updateItem(item.id, { rawName: event.target.value })} placeholder="Заполните название" /></td><td><input value={item.article} onChange={(event) => updateItem(item.id, { article: event.target.value })} placeholder="Артикул" /></td><td><div className="procurement-size-inputs"><input aria-label="Горшок, см" type="number" min="0" value={item.potDiameterCm} placeholder="P, см" onChange={(event) => updateItem(item.id, { potDiameterCm: draftNumber(event.target.value) })} /><input aria-label="Высота, см" type="number" min="0" value={item.heightCm} placeholder="H, см" onChange={(event) => updateItem(item.id, { heightCm: draftNumber(event.target.value) })} /></div></td><td><div className="procurement-quantity-inputs"><input aria-label="Количество упаковок" type="number" min="1" value={item.packageCount} placeholder="Кол-во" onChange={(event) => updateItem(item.id, { packageCount: draftNumber(event.target.value) })} /><span>×</span><input aria-label="Штук в упаковке" type="number" min="1" value={item.unitsPerPackage} placeholder="Кратность" onChange={(event) => updateItem(item.id, { unitsPerPackage: draftNumber(event.target.value) })} /></div><small>= {units || 0} шт.</small></td><td><input aria-label="Цена в евро" className="procurement-price-input" type="number" min="0" step="0.01" value={item.expectedUnitPrice} placeholder="0,00" onChange={(event) => updateItem(item.id, { expectedUnitPrice: draftNumber(event.target.value) })} /></td><td>{estimate ? <><strong>Себестоимость {money.format(estimate.cost)}</strong><small>Розница ≈ {money.format(estimate.retail)}</small><small>Закупка {money.format(estimate.purchase)}</small></> : <small>{valid ? (preview?.key === requestBody && preview.error ? preview.error : "Рассчитываем…") : "Заполните цену, горшок, высоту и упаковки во всех строках"}</small>}</td><td><button className="table-action danger" onClick={() => setItems((current) => current.filter((line) => line.id !== item.id))}>Удалить</button></td></tr>; })}</tbody></table></div>}
    <p className="admin-hint">Предварительная оценка: до Москвы — по объёму растений, до Рязани — по высоте. После PDF расчёт уточняется по фактическим телегам. Остатки и цены не меняются.</p><div className="dialog-actions procurement-plan-footer"><button onClick={onClose}>Отмена</button><button className="primary" disabled={!supplierId || !valid || saving} onClick={save}>{saving ? "Считаем…" : `Создать и рассчитать · ${selected.length}`}</button></div>
    {showRecommendations && <div className="procurement-recommendation-drawer" role="dialog" aria-modal="true" aria-label="Рекомендации к закупке"><header><div><p className="eyebrow">Что заказать</p><h3>Рекомендации к закупке</h3></div><button onClick={() => setShowRecommendations(false)} aria-label="Закрыть рекомендации">×</button></header><div className="procurement-recommendation-list">{availableRecommendations.length ? availableRecommendations.map((item) => <article key={item.sabyId}><div><strong>{item.name}</strong><small>Рекомендуется {item.suggestedQty} шт.</small></div><button className="admin-primary" onClick={() => addRecommendation(item)}>Добавить</button></article>) : <div className="procurement-plan-empty"><strong>Все рекомендации добавлены</strong><span>Можно закрыть окно и продолжить редактирование заказа.</span></div>}</div><div className="dialog-actions"><button className="primary" onClick={() => setShowRecommendations(false)}>Готово</button></div></div>}
  </div></>;
}
