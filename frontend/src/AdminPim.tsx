...
export function VariantsEditor({ productId, categoryId, onError }: { productId: number; categoryId?: number; onError: (value: string) => void }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = JSON.stringify({ label: draft.label, priceMinor: Math.round(draft.price * 100), stock: draft.stock, wholesaleMinQty: draft.wholesaleMinQty, active: draft.active, attributes, externalIds: draft.externalIds });
  ...
  return <section className="variant-editor wide">
    ...
    {selectedId && <div className="variant-detail"><div className="variant-summary"><strong>SKU {draft.sku}</strong><span>Заполнение схемы: {completeness}%</span></div>{draft.active && missingRequired.length > 0 && <p className="admin-inline-error" role="alert">Для активного SKU заполните: {missingRequired.map((item) => item.name).join(", ")}.</p>}<div className="admin-form-grid">
      ...
      <label>Цена, ₽<input type="number" min="0" disabled={draft.managedFields.includes("price")} value={draft.price} onChange={(event) => setDraft({ ...draft, price: Number(event.target.value) })} />{draft.managedFields.includes("price")&&<small>Управляет СБИС</small>}</label>
      <label>Остаток<input type="number" min="0" disabled={draft.managedFields.includes("stock")} value={draft.stock} onChange={(event) => setDraft({ ...draft, stock: Number(event.target.value) })} />{draft.managedFields.includes("stock")&&<small>Управляет СБИС</small>}</label>
