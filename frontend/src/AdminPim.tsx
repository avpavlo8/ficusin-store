...
export type AdminVariant = {
  ...
  externalIds: VariantMapping[]; images: string[]; managedFields: string[];
};
...
function DynamicValue({ attribute, value, onChange }: { attribute: EffectiveAttribute; value: unknown; onChange: (value: unknown) => void }) {
  ...
  if (attribute.code === "package_weight_grams") {
    const kilograms = value == null || value === "" ? "" : String(Number(value) / 1000);
    return <label>Вес упаковки, кг{attribute.required ? " *" : ""}<input type="number" min="0.01" step="0.01" inputMode="decimal" value={kilograms} onChange={(event) => onChange(event.target.value === "" ? null : Math.round(Number(event.target.value.replace(",", ".")) * 1000))} /><small>Технический ключ: <code>{attribute.code}</code></small></label>;
  }
  ...
  if (attribute.dataType === "enum") { const current=String(value??"");const options=attribute.options.filter((item)=>item.active);const unknown=current&&!options.some((item)=>item.code===current);return <label>{title}<select value={current} onChange={(event) => onChange(event.target.value || null)}><option value="">Не указано</option>{unknown&&<option value={current}>{`Неизвестное значение: ${current}`}</option>}{options.map((item) => <option value={item.code} key={item.code}>{item.label}</option>)}</select>{unknown&&<small className="admin-inline-error">Значение сохранено до явной замены.</small>}</label>; }
  if (attribute.dataType === "multi_enum") {
    const selected = Array.isArray(value) ? value.map(String) : [];
    const options=attribute.options.filter((item)=>item.active);const unknown=selected.filter((code)=>!options.some((item)=>item.code===code));
    return <fieldset className="attribute-multi"><legend>{title}</legend>{options.map((item) => <label key={item.code}><input type="checkbox" checked={selected.includes(item.code)} onChange={(event) => onChange(event.target.checked ? [...selected, item.code] : selected.filter((code) => code !== item.code))} />{item.label}</label>)}{unknown.length>0&&<small className="admin-inline-error">Старые значения сохранены: {unknown.join(", ")}</small>}</fieldset>;
  }
  const isPackageSize = /^package_(length|width|height)_cm$/.test(attribute.code);
  return <label>{title}<input type={attribute.dataType === "number" ? "number" : "text"} min={attribute.dataType === "number" ? (isPackageSize ? "0.1" : "0") : undefined} step={attribute.dataType === "number" ? "0.1" : undefined} inputMode={attribute.dataType === "number" ? "decimal" : undefined} value={value == null ? "" : String(value)} onChange={(event) => onChange(event.target.value === "" ? null : attribute.dataType === "number" ? Number(event.target.value.replace(",", ".")) : event.target.value)} />{isPackageSize&&<small>Технический ключ: <code>{attribute.code}</code></small>}</label>;
}
...
export function VariantsEditor({ owner = false, productId, categoryId, onError }: { owner?: boolean; productId: number; categoryId?: number; onError: (value: string) => void }) {
  const [variants, setVariants] = useState<AdminVariant[]>([]);
  const [schema, setSchema] = useState<EffectiveAttribute[]>([]);
  const [selectedId, setSelectedId] = useState<number | "new" | null>(null);
  const [saving, setSaving] = useState(false);
  const [saveNotice, setSaveNotice] = useState("");
  const empty = useMemo<AdminVariant>(() => ({ id: 0, productId, sku: "авто", label: "Новый вариант", price: 0, stock: 0, wholesaleMinQty: 1, active: true, archived: false, attributes: {}, externalIds: [], images: [], managedFields: [] }), [productId]);
  ...
  const save = async () => {
    ...
    try {
      const original = variants.find((item) => item.id === draft.id);
      const attributes: Record<string, unknown> = {};
      visibleSchema.forEach((attribute) => { const value=draft.attributes[attribute.code];if(original&&JSON.stringify(value)===JSON.stringify(original.attributes[attribute.code]))return;if(value==null||value===""||(Array.isArray(value)&&!value.length)){attributes[attribute.code]=null;return;}const values=Array.isArray(value)?value.map(String):[String(value)];if((attribute.dataType==="enum"||attribute.dataType==="multi_enum")&&values.some((item)=>!attribute.options.some((option)=>option.active&&option.code===item)))return;attributes[attribute.code]=value; });
      const body = JSON.stringify(owner ? { label: draft.label, priceMinor: Math.round(draft.price * 100), stock: draft.stock, wholesaleMinQty: draft.wholesaleMinQty, active: draft.active, attributes, externalIds: draft.externalIds } : {label: draft.label, attributes});
  ...
  const visibleSchema = categoryId ? schema.filter(item => owner || item.audience === "customer" || ["package_length_cm","package_width_cm","package_height_cm","package_weight_grams"].includes(item.code)) : [];
  ...
  const plantDimensions = variantAttributes.filter((item) => /(height|pot|diameter|size)/.test(item.code));
  const variantDetails = variantAttributes.filter((item) => !plantDimensions.includes(item));
  ...
  return <section className="variant-editor wide">
    ...
    {selectedId && <div className="variant-detail"><div className="variant-summary"><strong>SKU {draft.sku}</strong><span>Заполнение схемы: {completeness}%</span></div>{draft.active && missingRequired.length > 0 && <p className="admin-inline-error" role="alert">Для активного SKU заполните: {missingRequired.map((item) => item.name).join(", ")}.</p>}<div className="admin-form-grid">
      ...
      <label>Цена, ₽<input type="number" min="0" disabled={!owner || draft.managedFields.includes("price")} value={draft.price} onChange={(event) => setDraft({ ...draft, price: Number(event.target.value) })} />{draft.managedFields.includes("price")&&<small>Управляет СБИС</small>}</label>
      <label>Остаток<input type="number" min="0" disabled={!owner || draft.managedFields.includes("stock")} value={draft.stock} onChange={(event) => setDraft({ ...draft, stock: Number(event.target.value) })} />{draft.managedFields.includes("stock")&&<small>Управляет СБИС</small>}</label>
      <label>Оптовый минимум<input type="number" min="1" disabled={!owner} value={draft.wholesaleMinQty} onChange={(event) => setDraft({ ...draft, wholesaleMinQty: Number(event.target.value) })} /></label>
      <label className="admin-checkbox"><input type="checkbox" disabled={!owner} checked={draft.active} onChange={(event) => setDraft({ ...draft, active: event.target.checked })} />Активен</label>
      {plantDimensions.length > 0 && <><h4 className="wide">Размеры растения</h4>{plantDimensions.map((attribute) => <DynamicValue key={attribute.id} attribute={attribute} value={draft.attributes[attribute.code]} onChange={(value) => setDraft({ ...draft, attributes: { ...draft.attributes, [attribute.code]: value as never } })} />)}</>}
      {variantDetails.length > 0 && <><h4 className="wide">Характеристики SKU</h4>{variantDetails.map((attribute) => <DynamicValue key={attribute.id} attribute={attribute} value={draft.attributes[attribute.code]} onChange={(value) => setDraft({ ...draft, attributes: { ...draft.attributes, [attribute.code]: value as never } })} />)}</>}
      {technicalAttributes.length > 0 && <><h4 className="wide">Размеры и вес упаковки</h4><p className="wide admin-hint">Габариты относятся к одной единице товара и используются для расчёта доставки.</p>{technicalAttributes.map((attribute) => <DynamicValue key={attribute.id} attribute={attribute} value={draft.attributes[attribute.code]} onChange={(value) => setDraft({ ...draft, attributes: { ...draft.attributes, [attribute.code]: value as never } })} />)}</>}
      <h4 className="wide">Интеграции SKU</h4><div className="wide external-mappings">{draft.externalIds.map((mapping, index) => {const protectedMapping=!owner||mapping.provider==="saby"||mapping.provider==="ficusin";return <div className="external-mapping-row" key={`${index}-${mapping.provider}`}><input disabled={protectedMapping} value={mapping.provider} placeholder="wildberries / ozon" onChange={(event) => setDraft({ ...draft, externalIds: draft.externalIds.map((item, itemIndex) => itemIndex === index ? { ...item, provider: event.target.value.toLowerCase() } : item) })} /><input disabled={protectedMapping} value={mapping.type} placeholder="vendor_code / offer_id" onChange={(event) => setDraft({ ...draft, externalIds: draft.externalIds.map((item, itemIndex) => itemIndex === index ? { ...item, type: event.target.value } : item) })} /><input disabled={protectedMapping} value={mapping.externalId} placeholder="Артикул" onChange={(event) => setDraft({ ...draft, externalIds: draft.externalIds.map((item, itemIndex) => itemIndex === index ? { ...item, externalId: event.target.value } : item) })} />{protectedMapping?<small>Системная связь</small>:<button type="button" onClick={() => setDraft({ ...draft, externalIds: draft.externalIds.filter((_, itemIndex) => itemIndex !== index) })}>В legacy</button>}</div>;})}<p className="admin-hint">Для WB здесь хранится артикул продавца. Числовой nmID подтягивается автоматически и остаётся скрытым техническим ключом API.</p><button type="button" className="admin-action" disabled={!owner} onClick={() => setDraft({ ...draft, externalIds: [...draft.externalIds, { provider: "wildberries", type: "vendor_code", externalId: "" }] })}>Добавить артикул</button></div>
      <h4 className="wide">Фото SKU</h4>{selectedId === "new" ? <p className="wide admin-hint">Сначала сохраните SKU — после этого можно загрузить его фотографии.</p> : <VariantMediaManager canDelete={owner} variantId={draft.id} sku={draft.sku} onChanged={() => { void load(); }} onError={onError} />}
    </div><div className={`variant-save-status ${saveNotice ? "success" : ""}`} role="status" aria-live="polite">{saveNotice || "После изменений нажмите «Сохранить SKU»"}</div><div className="dialog-actions"><button type="button" className="danger" disabled={!owner || selectedId === "new" || saving} onClick={async () => { if (selectedId === "new") return; try { await api(`/api/v1/admin/variants/${draft.id}/archive`, { method: "POST" }); setSelectedId(null); load(); } catch (error) { onError((error as Error).message); } }}>Архивировать</button><button type="button" className="danger" disabled={!owner || selectedId === "new" || saving} onClick={async () => { if (selectedId === "new" || !window.confirm(`Удалить SKU ${draft.sku}? Это возможно только если он никогда не продавался.`)) return; try { await api(`/api/v1/admin/variants/${draft.id}`, { method: "DELETE" }); setSelectedId(null); load(); } catch (error) { onError((error as Error).message); } }}>Удалить</button><button type="button" className="primary" disabled={saving || draft.active && missingRequired.length > 0} onClick={save}>{saving ? "Сохраняем…" : saveNotice ? "Сохранено ✓" : "Сохранить SKU"}</button></div></div>}
  </section>;
}
