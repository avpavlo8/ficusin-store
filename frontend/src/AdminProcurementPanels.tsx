...
export function ProcurementProducts({ suppliers, onError }: { suppliers: ProcurementSupplier[]; onError: (value: string) => void }) {
  ...
  return <section className="admin-block procurement-block"><div className="admin-block-heading"><div><p className="eyebrow">Источник истины</p><h2>Товары и связи каналов</h2></div><span className="admin-pill">{items.length} товаров</span></div>
    <p className="admin-hint procurement-note">Одна строка — одна активная карточка СБИС. Здесь проверяются связи с сайтом, маркетплейсами и названиями поставщика; эти связи используются в продажах, рекомендациях и закупке.</p>
    ...
    {items.length ? <div className="admin-table-wrap"><table className="admin-table procurement-directory"><thead><tr><th>Товар</th><th>СБИС</th><th>WB / продажи</th><th>Ozon / продажи</th><th>Названия в инвойсах</th><th>Поставщик</th><th></th></tr></thead><tbody>{items.map((item) => <tr key={`${item.supplierId}-${item.variantId}`}><td><strong>{item.name}</strong><small>Товар сайта · вариант {item.variantId}</small><small>Продажи: магазин {item.sabySales} · сайт {item.siteSales}</small></td><td><strong>{item.sabyCode || "Код не заполнен"}</strong><small>ID: {item.sabyId}</small><small>Артикул: {item.sabyArticle || "—"}</small><small>Остаток: {item.balance}</small></td><td>{(item.wbArticles || []).map((article) => <small key={`wb-${article}`}>{article}</small>)}{!(item.wbArticles || []).length && <small>Не связан</small>}<strong>{item.wbSales} продаж</strong>{(item.wbLegacyArticles || []).map((article) => <small key={`wb-legacy-${article}`}>Архивный: {article}</small>)}</td><td>{(item.ozonArticles || []).map((article) => <small key={`ozon-${article}`}>{article}</small>)}{!(item.ozonArticles || []).length && <small>Не связан</small>}<strong>{item.ozonSales} продаж</strong>{(item.ozonLegacyArticles || []).map((article) => <small key={`ozon-legacy-${article}`}>Архивный: {article}</small>)}</td><td>{item.aliases.map((alias, index) => <small key={item.aliasIds[index] || `${alias}-${index}`}>{alias}</small>)}{!item.aliases.length && <small>Нет названий</small>}</td><td><strong>{item.supplierName}</strong><small>{item.supplierArticle || item.hollandArticle || "Артикул не заполнен"}</small><small>{availabilityLabel(item.availabilityStatus)}</small></td><td><button className="table-action" onClick={() => setEditing(item)}>Изменить</button></td></tr>)}</tbody></table></div> : <div className="procurement-zero"><strong>Товары не найдены</strong><span>Обновите каталог СБИС или измените строку поиска.</span></div>}
