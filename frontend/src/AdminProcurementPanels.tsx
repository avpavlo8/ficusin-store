...
export function ProcurementProducts({ suppliers, onError }: { suppliers: ProcurementSupplier[]; onError: (value: string) => void }) {
  ...
  return <section className="admin-block procurement-block"><div className="admin-block-heading"><div><p className="eyebrow">Канонические товары</p><h2>Справочник закупки</h2></div><span className="admin-pill">{items.length} товаров</span></div>
    ...
    {items.length ? <div className="admin-table-wrap"><table className="admin-table procurement-directory"><thead><tr><th>Наш товар</th><th>Поставщик</th><th>Артикулы каналов</th><th>Сопоставления инвойса</th><th>Заказ</th><th>Наличие</th><th></th></tr></thead><tbody>{items.map((item) => <tr key={`${item.supplierId}-${item.variantId}`}><td><strong>{item.name}</strong><small>Главный код: {item.sabyCode || "—"}</small><small>Карточка СБИС · остаток {item.balance}</small></td><td><strong>{item.supplierArticle || item.hollandArticle || "Артикул не заполнен"}</strong><small>{item.supplierName}</small></td><td>{(item.wbArticles || []).map((article) => <small key={`wb-${article}`}>WB: {article}</small>)}{!(item.wbArticles || []).length && <small>WB: —</small>}{(item.wbLegacyArticles || []).map((article) => <small key={`wb-legacy-${article}`}>WB legacy: {article}</small>)}{(item.ozonArticles || []).map((article) => <small key={`ozon-${article}`}>Ozon: {article}</small>)}{!(item.ozonArticles || []).length && <small>Ozon: —</small>}{(item.ozonLegacyArticles || []).map((article) => <small key={`ozon-legacy-${article}`}>Ozon legacy: {article}</small>)}</td><td>{item.aliases.slice(0, 3).map((alias, index) => <small key={item.aliasIds[index] || `${alias}-${index}`}>Из инвойса: {alias}</small>)}{item.aliases.length > 3 && <small>ещё {item.aliases.length - 3}</small>}</td><td><small>минимум {item.minimumOrderQty}</small><small>кратно {item.orderMultiple}</small></td><td>{availabilityLabel(item.availabilityStatus)}</td><td><button className="table-action" onClick={() => setEditing(item)}>Изменить</button></td></tr>)}</tbody></table></div> : <div className="procurement-zero"><strong>Товары не найдены</strong><span>Импортируйте карточки СБИС в единый справочник и добавьте сопоставления инвойсов.</span></div>}
