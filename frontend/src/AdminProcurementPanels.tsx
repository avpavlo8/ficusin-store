...
export function ProcurementProductDialog({ item, onClose, onSaved, onError }: { item: ProcurementProduct; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog" role="dialog" aria-modal="true" aria-labelledby="product-directory-title"><header><div><p className="eyebrow">{item.supplierName}</p><h2 id="product-directory-title">{item.name}</h2></div><button onClick={onClose} aria-label="Закрыть">×</button></header><div className="admin-form-grid">
    ...
    <div className="wide admin-field"><span className="admin-field-label">Ключи из инвойсов</span>{draft.aliases.map((alias, index) => <div className="procurement-alias-edit-row" key={draft.aliasIds[index] || `${alias}-${index}`}><small>{alias}</small>{draft.aliasIds[index] && <button type="button" className="table-action" onClick={() => setEditingAlias(index)}>Исправить сопоставление</button>}</div>)}</div>
...
function ProcurementAliasReassign({ aliasId, aliasName, currentSabyId, onCancel, onSaved, onError }: { aliasId: number; aliasName: string; currentSabyId: string; onCancel: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  return <section className="wide procurement-alias-reassign"><div className="admin-block-heading"><div><small>Исправляется ключ инвойса</small><strong>{aliasName}</strong></div><button type="button" onClick={onCancel}>Отмена</button></div><label>Найти правильную карточку Saby<input value={query} onChange={(event) => setQuery(event.target.value)} autoFocus /></label><div className="procurement-candidates">{searching ? <span>Ищем…</span> : items.map((candidate) => <article key={candidate.sabyId}><div><strong>{candidate.name}</strong><span>{[candidate.code, candidate.article, `ID ${candidate.sabyId}`].filter(Boolean).join(" · ")}</span><small>Остаток: {candidate.balance}</small></div><button type="button" disabled={saving || candidate.sabyId === currentSabyId} onClick={() => void choose(candidate.sabyId)}>{candidate.sabyId === currentSabyId ? "Сейчас выбрано" : "Выбрать"}</button></article>)}</div></section>;
}
