...
type ProcurementOrderDetail = { order: ProcurementOrder; costs: { exchangeRate: number; trolleyCostCurrency: number; trolleyCostRub: number; deliveryToMoscowRub: number; deliveryToRyazanRub: number }; validation: ProcurementValidation; lines: ProcurementOrderLine[]; batches: ProcurementActionBatch[] };
...
function Procurement({ onError }: { onError: (value: string) => void }) {
  ...
  const [recommendationView, setRecommendationView] = useState<"recommended" | "already_ordered" | "check_availability" | "new_assortment">("recommended");
  ...
  const actionableRecommendations = data.recommendations.filter((item) => item.status === "recommended" || item.status === "new_assortment");
...
const recommendationStatusLabel = (value: string) => ({ recommended: "К заказу", new_assortment: "Новинки", already_ordered: "Уже заказано", check_availability: "Проверить наличие" }[value] || value);
const recommendationEmptyTitle = (value: string) => ({ recommended: "Дефицита нет", new_assortment: "Новых идей нет", already_ordered: "Товаров в пути нет", check_availability: "Проверять нечего" }[value] || "Список пуст");
const recommendationEmptyText = (value: string) => value === "recommended" ? "Текущий остаток и товары в пути покрывают рассчитанный спрос." : value === "new_assortment" ? "Добавьте ручную рекомендацию магазина и свяжите её с товаром СБИС." : value === "already_ordered" ? "Здесь появятся позиции из действующих закупок, чтобы не заказать их повторно." : "Здесь появится спрос на товары с неподтверждённым наличием у поставщика.";
...
function ProcurementProducts({ suppliers, onError }: { suppliers: ProcurementSupplier[]; onError: (value: string) => void }) {
  ...
  return <section className="admin-block procurement-block"><div className="admin-block-heading"><div><p className="eyebrow">Канонические товары</p><h2>Справочник закупки</h2></div><span className="admin-pill">{items.length} товаров</span></div>
    ...
    {items.length ? <div className="admin-table-wrap"><table className="admin-table procurement-directory"><thead><tr><th>Товар СБИС</th><th>Поставщик</th><th>Каналы</th><th>Ключи инвойса</th><th>Заказ</th><th>Наличие</th><th></th></tr></thead><tbody>{items.map((item) => <tr key={`${item.supplierId}-${item.sabyId}`}><td><strong>{item.name}</strong><small>{[item.sabyCode, item.sabyArticle].filter(Boolean).join(" · ")}</small></td><td><strong>{item.supplierArticle || item.hollandArticle || "Артикул не заполнен"}</strong><small>{item.supplierName}</small></td><td><small>WB nmID: {item.wbNmId || "—"}</small><small>WB артикул: {item.wbVendorCode || "—"}</small><small>Ozon: {item.ozonOfferId || "—"}</small></td><td>{item.aliases.slice(0, 3).map((alias) => <small key={alias}>{alias}</small>)}{item.aliases.length > 3 && <small>ещё {item.aliases.length - 3}</small>}</td><td><small>минимум {item.minimumOrderQty}</small><small>кратно {item.orderMultiple}</small></td><td>{availabilityLabel(item.availabilityStatus)}</td><td><button className="table-action" onClick={() => setEditing(item)}>Изменить</button></td></tr>)}</tbody></table></div> : <div className="procurement-zero"><strong>Товары не найдены</strong><span>Сопоставьте хотя бы одно название из инвойса с товаром СБИС.</span></div>}
    {editing && <ProcurementProductDialog item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load(); }} onError={onError} />}
  </section>;
}

function ProcurementProductDialog({ item, onClose, onSaved, onError }: { item: ProcurementProduct; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  const [draft, setDraft] = useState(item); const [saving, setSaving] = useState(false);
  ...
  return <><button className="admin-dialog-backdrop" aria-label="Закрыть" onClick={onClose} /><div className="admin-dialog" role="dialog" aria-modal="true" aria-labelledby="product-directory-title"><header><div><p className="eyebrow">{item.supplierName}</p><h2 id="product-directory-title">{item.name}</h2></div><button onClick={onClose} aria-label="Закрыть">×</button></header><div className="admin-form-grid">
    ...
    <div className="wide admin-field"><span className="admin-field-label">Ключи из инвойсов</span>{draft.aliases.map((alias) => <small key={alias}>{alias}</small>)}</div>
...
function ProcurementOrderDetailDialog({ orderId, onClose, onSaved, onError }: { orderId: number; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
...
const batchStatusLabel = (value: string) => ({ draft: "Черновик", processing: "Выполняется", completed: "Выполнено", partially_completed: "Выполнено частично", failed: "Ошибка", cancelled: "Отменено" }[value] || value);
const actionStatus = (item: ProcurementActionItem) => {
...
function ConfirmDialog({ title, text, confirmLabel, busy, danger, onCancel, onConfirm }: {
