...
export function AdminRevenue({ onOpen }: { onOpen: () => void }) {
  ...
  useEffect(() => {
    let current = true;
    api<RevenueSummary>(`/api/v1/admin/analytics?days=${days}&channel=all`).then(data => {
  ...
  return <section className="workspace-panel workspace-revenue" aria-label="Продажи по всем каналам">
    <header className="workspace-panel-heading"><div><p className="eyebrow">Продажи по всем каналам</p><h2>Динамика выручки</h2></div><div className="workspace-filters" role="group" aria-label="Период продаж">{[7, 30].map(value => <button type="button" key={value} aria-pressed={days === value} onClick={() => setDays(value)}>{value === 7 ? "Неделя" : "Месяц"}</button>)}</div></header>
