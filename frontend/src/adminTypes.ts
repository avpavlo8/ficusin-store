...
export type PnLReport = { from:string; to:string; preliminary:boolean; revenue:number; cogs:number; grossProfit:number; marketplaceCosts:number; packaging:number; returnLoss:number; operatingExpenses:number; profitBeforeTax:number; actualTax?:number; netProfit?:number; cashFlow:number; taxRegime:string; taxRate:number; actualCostUnits:number; estimatedCostUnits:number; unknownCostUnits:number; actualCostAmount:number; estimatedCostAmount:number; warnings:string[]|null; channels:Array<{channel:string;revenue:number;cogs:number;costs:number;profit:number}>|null; products:Array<{variantId:number;product:string;sku:string;units:number;revenue:number;cogs:number;packaging:number;costSource:string;estimated:boolean;unknownUnits:number}>|null };
...
export type SupplierAccountOverview = { balanceEur:number;originalBalanceEur?:number;reservedEur?:number;availableEur?:number;reconciliationDifferenceEur?:number;lastReconciledAt?:string;operations:SupplierAccountOperation[]|null };
