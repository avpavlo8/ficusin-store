...
function AvailabilityRow({item,reload,onError}:{item:ProcurementAvailability;reload:()=>Promise<unknown>;onError:(value:string)=>void}) {
  const [reason,setReason]=useState(item.reason||""); const [comment,setComment]=useState(item.comment||"");
  const [checkAfter,setCheckAfter]=useState(item.checkAfter||"");
  const save=(status:string)=>updateAvailability(item.supplierId,item.sabyId,status,reload,onError,{reason,comment,checkAfter:status==="available"?"":checkAfter});
...
export function Procurement({ onError }: { onError: (value: string) => void }) {
  ...
  const selectedRecommendationItems = actionableRecommendations.filter((item) => selectedRecommendations.includes(`${item.supplierId}:${item.sabyId}`));
  const selectedSupplierId = selectedRecommendationItems[0]?.supplierId;
  const availabilityItems = data.availability.filter((item) => !availabilityDueOnly || item.due);
  ...
  if (planDialog) return <ProcurementPlanDialog suppliers={data.suppliers} recommendations={selectedRecommendationItems} settings={data.settings} onClose={() => setPlanDialog(false)} onSaved={() => { setPlanDialog(false); setView("orders"); void load(); }} onError={onError} />;
  return <>
    ...
    {view === "recommendations" && <section className="admin-block procurement-block">
      ...
      {recommendationItems.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Выбрать</th><th>Товар</th><th>Поставщик / артикул</th><th>Остаток / в пути</th><th>Продажи</th><th>Хватит на</th><th>Запросы</th><th>Заказать</th><th></th></tr></thead><tbody>{recommendationItems.map((item) => { const key=`${item.supplierId}:${item.sabyId}`; const selectable=item.status === "recommended"; return <tr key={key}><td>{selectable && <input aria-label={`Выбрать ${item.name}`} type="checkbox" checked={selectedRecommendations.includes(key)} disabled={selectedSupplierId != null && selectedSupplierId !== item.supplierId} onChange={(event) => setSelectedRecommendations((current) => event.target.checked ? [...current,key] : current.filter((value) => value !== key))} />}</td><td><strong>{item.name}</strong><small>{item.reason}</small>{item.lastOrderedAt && <small>Последний заказ: {new Date(item.lastOrderedAt).toLocaleDateString("ru-RU")}</small>}</td><td><strong>{data.suppliers.find((supplier) => supplier.id === item.supplierId)?.name || "—"}</strong><small>{item.supplierArticle || "Артикул не заполнен"}</small></td><td><strong>{item.balanceKnown === false ? "Неизвестен" : item.balance} / {item.incoming}</strong><small>{item.balanceAsOf ? `СБИС: ${new Date(item.balanceAsOf).toLocaleString("ru-RU")}` : "Дата остатка неизвестна"}</small></td><td><strong>{item.totalSales}</strong><small>всего {item.grossSales ?? item.totalSales} − возвраты {item.returns ?? 0}</small><small>магазин {item.sabySales} · сайт {item.siteSales} · WB {item.wbSales} · Ozon {item.ozonSales}</small></td><td>{item.daysOfCover == null ? "Нет продаж" : `${Math.round(item.daysOfCover)} дн.`}</td><td>{item.openRequests}<small>{item.customerRequests ? `клиенту ${item.customerRequests}` : ""}{item.staffRequests ? `${item.customerRequests ? " · " : ""}магазину ${item.staffRequests}` : ""}</small></td><td><strong>{item.quantityKnown === false ? "Задать вручную" : `${item.suggestedQty} шт.`}</strong><small>{item.roundingExplanation || (item.demandQty == null ? `Минимум ${item.minimumOrderQty}, кратно ${item.orderMultiple}` : `Потребность ${item.demandQty} − остаток ${item.balance} − в пути ${item.incoming}`)}</small></td><td><div className="procurement-inline-actions">{item.status !== "check_availability" && item.status !== "excluded" && <button onClick={() => void updateAvailability(item.supplierId, item.sabyId, "check", load, onError)}>Проверить наличие</button>}{item.status !== "supplier_unavailable" && item.status !== "excluded" && <button onClick={() => void updateAvailability(item.supplierId, item.sabyId, "temporarily_unavailable", load, onError)}>Нет у поставщика</button>}{item.status === "excluded" ? <button onClick={() => void setExclusion(item.sabyId, false, "", load, onError)}>Вернуть в закупку</button> : <button onClick={() => void setExclusion(item.sabyId, true, "", load, onError)}>Не закупать</button>}</div></td></tr>})}</tbody></table></div> : <div className="procurement-zero"><strong>{recommendationEmptyTitle(recommendationView)}</strong><span>{recommendationEmptyText(recommendationView)}</span></div>}
    </section>}

    {view === "products" && <Suspense fallback={<section className="admin-block procurement-block"><p className="admin-hint">Загружаем каталог Saby…</p></section>}><ProcurementProducts suppliers={data.suppliers} onError={onError} /></Suspense>}
    ...
    {view === "requests" && <section className="admin-block procurement-block">
      ...
      {data.requests.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Тип</th><th>Товар</th><th>Количество</th><th>Статус</th><th>Комментарий</th><th>Действия</th></tr></thead><tbody>{data.requests.map((item) => <tr key={item.id}><td>{item.kind === "customer_order" ? <span className="procurement-warning">Клиенту</span> : "Рекомендация"}<small>{item.customerName || (item.customerOrderId ? `Заказ #${item.customerOrderId}` : item.source)}</small></td><td><strong>{item.requestedName}</strong><small>{item.sabyId || "Не связан со СБИС"}</small></td><td><strong>{item.quantity}</strong><small>распределено {item.allocatedQuantity} · осталось {item.remainingQuantity}</small></td><td>{item.status === "included" ? "В закупке" : "Открыт"}<small>{item.allocations?.map((allocation) => `${allocation.orderNumber || `№${allocation.procurementOrderId}`}: ${allocation.quantity} шт.`).join(" · ")}</small></td><td>{item.notes || "—"}</td><td><div className="procurement-inline-actions"><button onClick={() => void updateRequestStatus(item, "fulfilled", load, onError)}>Выполнен</button><button onClick={() => void updateRequestStatus(item, "cancelled", load, onError)}>Отменить</button></div></td></tr>)}</tbody></table></div> : <div className="procurement-zero"><strong>Запросов нет</strong><span>Добавьте клиентский заказ или рекомендацию сотрудника.</span></div>}
    </section>}

    {view === "availability" && <section className="admin-block procurement-block">
      <div className="admin-block-heading"><div><p className="eyebrow">Наличие у поставщика</p><h2>Проверить наличие</h2></div><div className="procurement-inline-actions"><button className={availabilityDueOnly ? "active" : ""} onClick={() => setAvailabilityDueOnly(true)}>Пора проверить</button><button className={!availabilityDueOnly ? "active" : ""} onClick={() => setAvailabilityDueOnly(false)}>Все</button></div></div>
