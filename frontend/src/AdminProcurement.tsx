...
function AvailabilityRow({item,reload,onError}:{item:ProcurementAvailability;reload:()=>Promise<unknown>;onError:(value:string)=>void}) {
  ...
  const [checkAfter,setCheckAfter]=useState(item.checkAfter||new Date(Date.now()+7*86400000).toISOString().slice(0,10));
...
export function Procurement({ onError }: { onError: (value: string) => void }) {
  ...
  return <>
    ...
    {view === "recommendations" && <section className="admin-block procurement-block">
      <div className="admin-block-heading"><div><p className="eyebrow">Остаток СБИС + продажи всех каналов</p><h2>Рекомендации к закупке</h2></div><button className="admin-primary" disabled={!selectedRecommendationItems.length || !data.suppliers.length} onClick={() => setPlanDialog(true)}>Сформировать заказ · {selectedRecommendationItems.length}</button></div>
