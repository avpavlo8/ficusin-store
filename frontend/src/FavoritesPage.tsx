...
type Product = { id: string; sku: string; name: string; latin: string; price: number; image: string; size: string; light: string; stock: number };
...
export default function FavoritesPage() {
  ...
  useEffect(() => { void sharedAPIJSON<{ products?: Product[] }>("/api/v1/catalog").then((data) => setProducts(data.products || [])).catch(() => setProducts([])); }, []);
  ...
  const add = (product: Product) => setCart((current) => ({ ...current, [product.sku]: Math.min(product.stock > 0 ? product.stock : 20, (current[product.sku] || 0) + 1) }));
    ...
    <section className="favorites-page"><div className="favorites-hero"><div><p className="eyebrow">Сохранённые товары</p><h1>Ваши любимые<br/>растения</h1><p>Сохраняйте находки и возвращайтесь к ним, когда будете готовы выбрать.</p></div></div>
      <div className="product-grid">{items.map((product) => <article className="product-card" key={product.id}>
        ...
        <div className="product-info"><p className="latin">{product.latin}</p><h3><a href={`/product/${product.id}`}>{product.name}</a></h3>
          ...
          <div className="product-bottom"><strong>{money(product.price)}</strong><button className={cart[product.sku] ? "in-cart" : undefined} onClick={() => add(product)}>{cart[product.sku] ? `Добавить ещё · ${cart[product.sku]} шт.` : "В корзину"}</button></div>
        </div></article>)}</div>
