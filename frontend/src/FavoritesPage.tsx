...
export default function FavoritesPage() {
  ...
  const add = (product: Product) => { const next = { ...cart, [product.id]: Math.min(product.stock, (cart[product.id] || 0) + 1) }; setCart(next); localStorage.setItem("ficusin-cart", JSON.stringify(next)); window.dispatchEvent(new Event(STORAGE_EVENT)); };
