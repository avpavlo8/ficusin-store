...
export function AdminShipmentOffers({orderId,items,offers,deliveryMethod,deliveryFee,cdekTariffCode,busy,setBusy,onCreated,onError}:{
  orderId:number;items:Item[];offers:Offer[];deliveryMethod:string;deliveryFee:number;cdekTariffCode?:number;busy:boolean;
  ...
  return <section className="admin-block admin-shipment-offers">
    ...
    {!active&&<div className="admin-shipment-builder">
    ...
    {offers.map((offer)=><article className="admin-shipment-offer" key={offer.id}>
      ...
      {offer.status==="draft"&&<button type="button" className="admin-action" disabled={busy} onClick={()=>void send(offer.id)}>Уведомить клиента</button>}
    </article>)}
