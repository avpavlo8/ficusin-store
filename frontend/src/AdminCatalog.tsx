...
export function Products({ can, onError }: { can: (permission: string) => boolean; onError: (value: string) => void }) {
  ...
  const [publishResult, setPublishResult] = useState<{ published: number[]; blocked: { productId: number; name: string; reason: string }[] } | null>(null);
  ...
  return <><PageHeading eyebrow="Каталог" title="Товары" text="Контент сайта, цены, упаковка, публикация и выборочная синхронизация со СБИС" />
    <div className="admin-category-tabs" role="group" aria-label="Раздел каталога"><button className={rootCategoryFilter === "all" ? "active" : ""} onClick={() => { setRootCategoryFilter("all"); setSelected([]); }}>Все товары</button>{categories.filter((item) => item.parentId === null).sort((left, right) => left.sortOrder - right.sortOrder).map((category) => <button className={rootCategoryFilter === category.id ? "active" : ""} onClick={() => { setRootCategoryFilter(category.id); setSelected([]); }} key={category.id}><span aria-hidden="true">{rootCategoryIcons[category.icon] || "•"}</span>{category.name}</button>)}<button className={rootCategoryFilter === "none" ? "active" : ""} onClick={() => { setRootCategoryFilter("none"); setSelected([]); }}>Без категории</button></div>
