...
import { readinessFor, readinessLabels, rootCategory as findRootCategory, type ProductReadiness, type ReadinessCode } from "./adminCatalogLogic";
import type { Category, CategoryAttribute, Product, ReviewModerationItem } from "./adminTypes";

const rootCategoryIcons: Record<string, string> = { leaf: "🌿", soil: "🪴", fertilizer: "💧", pot: "🏺", tools: "✂️" };
    ...
    <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Категория</th><th>Slug</th><th>Товары</th><th /></tr></thead><tbody>{visibleItems.map((item) => {
      ...
      return <tr key={item.id} className={canEdit ? "clickable" : ""} onClick={() => { if (canEdit) rename(item); }}><td><strong style={{ paddingLeft: depth(item) * 24 }}>
        {hasChildren
          ...
          : <span className="category-toggle placeholder" aria-hidden="true" />}
        {item.name}
      </strong></td><td><code>{item.slug}</code></td><td>{item.productsCount}</td><td>{owner && <button className="text-button danger" onClick={(event) => { event.stopPropagation(); remove(item); }}>Удалить</button>}</td></tr>;
    })}</tbody></table></div>
    {owner && <AttributeManager categories={items} onError={onError} />}
  </>;
}

export function Collections({ owner, onError }: { owner: boolean; onError: (value: string) => void }) {
  return <CollectionsV2 owner={owner} onError={onError} />;
}
...
export function Products({ can, onError }: { can: (permission: string) => boolean; onError: (value: string) => void }) {
  ...
  const [subcategoryFilter, setSubcategoryFilter] = useState<"all" | number>("all");
  const [readinessFilter, setReadinessFilter] = useState<"all" | ReadinessCode>("all");
  const [categories, setCategories] = useState<Category[]>([]);
  const [schemas, setSchemas] = useState<Record<number, CategoryAttribute[]>>({});
  const [selected, setSelected] = useState<number[]>([]);
  const [confirmingDelete, setConfirmingDelete] = useState(false);
  const [confirmingPublish, setConfirmingPublish] = useState(false);
  const [publishing, setPublishing] = useState(false);
  const [publishResult, setPublishResult] = useState<{ published: number[]; blocked: { productId: number; name: string; reason: string }[]; errors: { productId: number; message: string }[] } | null>(null);
  ...
  useEffect(() => { const ids = [...new Set(items.map((item) => item.categoryId).filter((id): id is number => Boolean(id)))].filter((id) => schemas[id] === undefined); if (!ids.length) return; let active = true; void Promise.all(ids.map(async (id) => [id, (await api<{attributes:CategoryAttribute[]}>(`/api/v1/admin/categories/${id}/attributes`)).attributes || []] as const)).then((entries) => { if (active) setSchemas((current) => ({...current,...Object.fromEntries(entries)})); }).catch((error) => onError((error as Error).message)); return () => { active = false; }; }, [items, schemas, onError]);
  ...
  const readiness = useMemo(() => new Map<number, ProductReadiness>(items.map((item) => [item.id, readinessFor(item, categories, item.categoryId ? schemas[item.categoryId] || [] : [])])), [items, categories, schemas]);
  const belongsTo = useCallback((categoryId: number | undefined, parentId: number) => { let current = categories.find((item) => item.id === categoryId); const visited = new Set<number>(); while (current && !visited.has(current.id)) { if (current.id === parentId) return true; visited.add(current.id); current = categories.find((item) => item.id === current?.parentId); } return false; }, [categories]);
  const scoped = useMemo(() => items.filter((item) => (statusFilter === "all" || item.status === statusFilter) && (stockFilter === "all" || (stockFilter === "positive" ? item.stock > 0 : item.stock <= 0)) && (rootCategoryFilter === "all" || (rootCategoryFilter === "none" ? !item.categoryId : findRootCategory(categories,item.categoryId)?.id === rootCategoryFilter)) && (subcategoryFilter === "all" || belongsTo(item.categoryId,subcategoryFilter)) && `${item.name} ${item.sku} ${item.sabyCode} ${item.externalIds.map((entry)=>entry.externalId).join(" ")}`.toLowerCase().includes(query.toLowerCase())), [items, query, statusFilter, stockFilter, rootCategoryFilter, subcategoryFilter, categories, belongsTo]);
  const filtered = useMemo(() => scoped.filter((item) => readinessFilter === "all" || readiness.get(item.id)?.codes.includes(readinessFilter)), [scoped, readinessFilter, readiness]);
  const counters = useMemo(() => ({ no_photo: scoped.filter((item)=>readiness.get(item.id)?.codes.includes("no_photo")).length, no_short_description: scoped.filter((item)=>readiness.get(item.id)?.codes.includes("no_short_description")).length, stage1_missing: scoped.filter((item)=>readiness.get(item.id)?.codes.includes("stage1_missing")).length, care_missing: scoped.filter((item)=>readiness.get(item.id)?.codes.includes("care_missing")).length, ready: scoped.filter((item)=>readiness.get(item.id)?.codes.includes("ready")).length }), [scoped,readiness]);
  ...
  const deleteDrafts = async () => {
    ...
    try {
      await api("/api/v1/admin/products", { method: "DELETE", body: JSON.stringify({ productIds: selected }) });
      setSelected([]); setConfirmingDelete(false); reload();
    } catch (error) { onError((error as Error).message); }
  };
  const publishDrafts = async () => {
    ...
    try {
      ...
      setPublishResult({ published, blocked, errors: [] }); setSelected(blocked.map((item) => item.productId)); setConfirmingPublish(false); reload();
    } catch (error) {
      setPublishResult({ published, blocked, errors: productIds.filter((id)=>!published.includes(id)&&!blocked.some((item)=>item.productId===id)).map((productId)=>({productId,message:(error as Error).message})) });
  ...
  return <><PageHeading eyebrow="Каталог" title="Товары" text="Контент сайта, цены, упаковка, публикация и выборочная синхронизация со СБИС" />
    <div className="admin-category-tabs" role="group" aria-label="Раздел каталога"><button className={rootCategoryFilter === "all" ? "active" : ""} onClick={() => { setRootCategoryFilter("all");setSubcategoryFilter("all"); setSelected([]); }}>Все товары</button>{categories.filter((item) => item.parentId === null).sort((left, right) => left.sortOrder - right.sortOrder).map((category) => <button className={rootCategoryFilter === category.id ? "active" : ""} onClick={() => { setRootCategoryFilter(category.id);setSubcategoryFilter("all"); setSelected([]); }} key={category.id}><span aria-hidden="true">{rootCategoryIcons[category.icon] || "•"}</span>{category.name}</button>)}<button className={rootCategoryFilter === "none" ? "active" : ""} onClick={() => { setRootCategoryFilter("none");setSubcategoryFilter("all"); setSelected([]); }}>Без категории</button></div>
    <div className="admin-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Название, SKU или внешний код" /><select aria-label="Подкатегория" value={subcategoryFilter} disabled={rootCategoryFilter==="none"} onChange={(event)=>{setSubcategoryFilter(event.target.value==="all"?"all":Number(event.target.value));setSelected([]);}}><option value="all">Все подкатегории</option>{categories.filter((item)=>item.parentId!==null&&(rootCategoryFilter==="all"||typeof rootCategoryFilter==="number"&&findRootCategory(categories,item.id)?.id===rootCategoryFilter)).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Готовность" value={readinessFilter} onChange={(event)=>{setReadinessFilter(event.target.value as typeof readinessFilter);setSelected([]);}}><option value="all">Любая готовность</option>{(Object.entries(readinessLabels) as Array<[ReadinessCode,string]>).map(([code,label])=><option key={code} value={code}>{label}</option>)}</select><select aria-label="Статус карточки" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value as typeof statusFilter); setSelected([]); setConfirmingDelete(false); setConfirmingPublish(false); setPublishResult(null); }}><option value="all">Все статусы</option><option value="draft">Черновики ({items.filter((item) => item.status === "draft").length})</option><option value="published">Опубликованные</option><option value="archived">Архив</option></select><select aria-label="Остаток" value={stockFilter} onChange={(event) => { setStockFilter(event.target.value as typeof stockFilter); setSelected([]); setConfirmingDelete(false); setConfirmingPublish(false); setPublishResult(null); }}><option value="all">Любой остаток</option><option value="positive">Есть в наличии</option><option value="nonpositive">Нет в наличии</option></select><span>{selected.length ? `Выбрано: ${selected.length}` : `${filtered.length} товаров`}</span>{selected.length >= 2 && selectedAreDrafts && can("products.manage") && <button onClick={() => setMerging(items.filter((item) => selected.includes(item.id)))}>Объединить размеры</button>}{selectedAreDrafts && can("products.edit") && (confirmingPublish ? <button className="admin-primary" disabled={publishing} onClick={() => void publishDrafts()}>{publishing ? "Публикуем…" : `Подтвердить публикацию (${selected.length})`}</button> : <button className="admin-primary" onClick={() => { setConfirmingPublish(true); setConfirmingDelete(false); }}>Опубликовать</button>)}{selectedAreDrafts && can("catalog.delete") && (confirmingDelete ? <button className="admin-danger" onClick={() => void deleteDrafts()}>Подтвердить удаление ({selected.length})</button> : <button className="admin-danger" onClick={() => { setConfirmingDelete(true); setConfirmingPublish(false); }}>Удалить черновики</button>)}{selected.length > 0 && can("products.sync") && <button onClick={() => setSyncing(selected)}>Подтянуть из СБИС</button>}{can("products.manage") && <button onClick={() => setImporting(true)}>Импорт из СБИС</button>}{can("products.manage") && <button className="admin-primary" onClick={() => setCreating(true)}>Новый товар</button>}</div>
    <div className="admin-readiness-counters"><button onClick={()=>setReadinessFilter("no_photo")}>Без фото <b>{counters.no_photo}</b></button><button onClick={()=>setReadinessFilter("no_short_description")}>Без описания <b>{counters.no_short_description}</b></button><button onClick={()=>setReadinessFilter("stage1_missing")}>Без обязательных характеристик <b>{counters.stage1_missing}</b></button><button onClick={()=>setReadinessFilter("care_missing")}>Без ухода <b>{counters.care_missing}</b></button><button onClick={()=>setReadinessFilter("ready")}>Готово <b>{counters.ready}</b></button></div>
    {publishResult && <div className={`admin-bulk-result ${publishResult.blocked.length||publishResult.errors.length ? "warning" : "success"}`}><strong>Успешно: {publishResult.published.length} · Пропущено: {publishResult.blocked.length} · Ошибка: {publishResult.errors.length}</strong>{publishResult.blocked.length > 0 && <><span>Пропущенные карточки остались выделенными.</span><ul>{publishResult.blocked.slice(0, 20).map((item) => <li key={item.productId}><b>{item.name || `Товар #${item.productId}`}</b> — {item.reason}</li>)}</ul>{publishResult.blocked.length > 20 && <small>И ещё {publishResult.blocked.length - 20}. Исправьте показанные причины и повторите публикацию.</small>}</>}{publishResult.errors.slice(0,20).map((item)=><p className="admin-inline-error" key={item.productId}>Товар #{item.productId}: {item.message}</p>)}</div>}
    ...
    >
      ...
      <td><div className="admin-product"><img src={product.image || "/assets/hero-monstera.webp"} alt="" /><div><strong>{product.name}</strong><small>{product.sku} · {product.variantLabel}</small><a href={`/product/${product.slug}`} target="_blank" onClick={(event) => event.stopPropagation()}>Открыть карточку ↗</a></div></div></td>
      ...
      <td><span className={`admin-pill ${product.status}`}>{statusLabels[product.status] || product.status}</span><div className="readiness-pills">{readiness.get(product.id)?.codes.slice(0,3).map((code)=><small key={code}>{readinessLabels[code]}</small>)}</div>{product.overrideFields.length > 0 && <small>Изменено вручную: {product.overrideFields.join(", ")}</small>}</td>
    ...
    <section className="admin-block review-moderation"><div className="admin-block-heading"><div><p className="eyebrow">Обратная связь</p><h2>Все отзывы</h2></div><span className="admin-pill">{reviews.length}</span></div>{reviews.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Товар / покупатель</th><th>Оценка</th><th>Отзыв и медиа</th><th>Статус</th></tr></thead><tbody>{reviews.map((review) => <tr key={review.id}><td><strong>{review.product}</strong><small>{review.author} · {new Date(review.createdAt).toLocaleDateString("ru-RU")}</small></td><td><span className="review-stars">{'★'.repeat(review.rating)}{'☆'.repeat(5-review.rating)}</span></td><td><p>{review.text}</p>{review.media?.length > 0 && <div className="admin-review-media">{review.media.map((media) => media.contentType.startsWith("video/") ? <video key={media.url} src={media.url} controls preload="metadata" /> : <img key={media.url} src={media.url} alt="Вложение к отзыву" />)}</div>}</td><td><div className="admin-review-actions"><span className={`admin-pill ${review.status}`}>{review.status === "published" ? "Опубликован" : "Скрыт"}</span>{review.status === "published" ? <button disabled={!can("products.edit")} onClick={() => void moderate(review.id, "rejected")}>Скрыть</button> : <button className="admin-primary" disabled={!can("products.edit")} onClick={() => void moderate(review.id, "published")}>Вернуть на сайт</button>}</div></td></tr>)}</tbody></table></div> : <p className="admin-hint">Отзывов пока нет.</p>}</section>
    {editing && <ProductDialog owner={can("products.manage")} key={editing.id} product={editing} hasNext={filtered.some((item)=>item.id!==editing.id&&(readiness.get(item.id)?.blockers.length||0)>0)} onClose={() => setEditing(null)} onSaved={(product, openNext) => { replace(product); if (!openNext) { setEditing(null); return; } const index=filtered.findIndex((item)=>item.id===product.id);const ordered=[...filtered.slice(index+1),...filtered.slice(0,Math.max(index,0))];setEditing(ordered.find((item)=>(readiness.get(item.id)?.blockers.length||0)>0)||product); }} onError={onError} />}
