...
export function Products({ can, onError }: { can: (permission: string) => boolean; onError: (value: string) => void }) {
  ...
  const deleteDrafts = async () => {
    if (!selectedAreDrafts || !window.confirm(`Удалить выбранные черновики (${selected.length})? Это действие нельзя отменить.`)) return;
    try {
  ...
  return <><PageHeading eyebrow="Каталог" title="Товары" text="Контент сайта, цены, упаковка, публикация и выборочная синхронизация со СБИС" />
    <div className="admin-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Название, SKU или код СБИС" /><select aria-label="Статус карточки" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value as typeof statusFilter); setSelected([]); }}><option value="all">Все статусы</option><option value="draft">Черновики ({items.filter((item) => item.status === "draft").length})</option><option value="published">Опубликованные</option><option value="archived">Архив</option></select><select aria-label="Остаток" value={stockFilter} onChange={(event) => { setStockFilter(event.target.value as typeof stockFilter); setSelected([]); }}><option value="all">Любой остаток</option><option value="positive">Есть в наличии</option><option value="nonpositive">Нет в наличии</option></select><span>{selected.length ? `Выбрано: ${selected.length}` : `${filtered.length} товаров`}</span>{selected.length >= 2 && selectedAreDrafts && can("products.edit") && <button onClick={() => setMerging(items.filter((item) => selected.includes(item.id)))}>Объединить размеры</button>}{selectedAreDrafts && can("products.edit") && <button className="admin-danger" onClick={() => void deleteDrafts()}>Удалить черновики</button>}{selected.length > 0 && can("products.sync") && <button className="admin-primary" onClick={() => setSyncing(selected)}>Подтянуть из СБИС</button>}{can("products.edit") && <button onClick={() => setImporting(true)}>Импорт из СБИС</button>}{can("products.edit") && <button className="admin-primary" onClick={() => setCreating(true)}>Новый товар</button>}</div>
