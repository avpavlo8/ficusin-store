...
-- Keep marketplace mappings current after managers refresh channel cards.
-- Old non-empty IDs remain as historical aliases; an empty integration value
-- never deletes a previously confirmed mapping.
...
BEGIN
  ...
  RETURN NEW;
END $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS procurement_channels_sync_external_ids ON procurement_product_channels;
CREATE TRIGGER procurement_channels_sync_external_ids AFTER INSERT OR UPDATE OF
  wb_nm_id,wb_vendor_code,ozon_offer_id ON procurement_product_channels
FOR EACH ROW EXECUTE FUNCTION sync_marketplace_external_ids();
